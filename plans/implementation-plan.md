...
- **The one queue (replanned 2026-09-20; design record
  ...
  | P6 **(delivered: PR #277; live witness run with finding)** | Phase 8 gate witness: two Runtimes execute independent offline runs, reconnect, sync through P5, and attempt the same shared push; the second is refused without the P3 lease. Driver test green (1227/0). Live witness on kxm-dev-svr: run completed, 23 sync events pushed (0 pending), runtime presence registered — **finding: the supervisor registers presence under the npm package name (`@kontextmind/kxm`) while sync events carry the project.yaml ID (`prj_kxm_project`), so the ops snapshot cannot join them. Fix landed (#280). **Open**: the sync loop runs but does not push pending outbox rows — the supervisor logs clean (no errors, `ready`) yet 23 rows stay pending after restart. The presence fix is verified (runtime visible in the snapshot under the correct project); the push path needs debugging (hub client creation, sync tick error handling, or the contexts map after restart recovery). Recorded as the next slice before the Phase 8 gate is marked fulfilled | P3, P5 | driver green; live witness records both run ids, the sync cursor and the refused push | `two runtimes synchronize independent offline runs and a conflicting shared push is refused without the lease` in `test/core/driver.test.ts` |
