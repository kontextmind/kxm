...
- **The one queue (replanned 2026-09-20; design record
  ...
  | S2 **(KXM-side read delivered; portal pieces open)** | Portal reads authoritative state: tenant label, connectivity, agents, runs, current status, latest receipt — hub metadata distinguished from Runtime run state, stale/unavailable explicit, polling. `kxm tenant status` (PR pending) composes both labelled authorities with concurrent bounded reads, folds Runtime rows through the event log, attaches to (never starts) the supervisor, resolves admin credentials only, and reports the hub/Runtime id-space cross-check as `unverified` when no run id appears in both; remaining: the `ws-api.py` route on the tenant box (shell out + cache) and the `kxmd-portal` card — **canonical front end is `kxmd-portal`; `dev-vm-platform/ops/pilot/web` is a legacy snapshot** (their docs/16) | S1, portal router access | the read plus a browser comparison against the same run's CLI/API state | `portal reads distinguish hub metadata from Runtime run state and unavailable upstreams` in `test/core/studio-layout.test.ts` (landed) |
