...
- **The one queue (replanned 2026-09-20; design record
  ...
  | S5 **(in progress on the deployed box)** | Edge authentication and first real use + the deployed restore witness. Done on kxm-dev-svr: box upgraded to current main (hub + Runtime as systemd services, S1-conformant declared paths — the hub DB had sat at the cwd-inherited `~/.kxm/state`), `kxm tenant status` first live use (both sources, honest cross-check), **restore witness passed** (manifest restore `bk_f69b6f4684b0dc19` + whole-set tarball restore with services restarted and state verified), and #262 (agent-declared model routes, found live on the box). **Open:** the live drive executes its step on `driver-simulated` despite `mode: live` (routing record proves it; run then failed `outcome_unknown`) — follow-up issue before the "one real run" witness; then edge HTTPS+Authentik (tenant-admin only) | S4, tenant DNS/TLS/Authentik config | deployed witness: restore ✅; real run pending the live-producer fix; edge auth pending | none — named deployment witness, no new suite |
