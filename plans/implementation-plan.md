...
- **The one queue (replanned 2026-09-20; design record
  ...
  | S4 **(kxm parity witness landed; routes/UI open)** | Portal drives one workflow: create, drive, cancel only, reusing existing command/run/drive IDs and receipts; 202 is started, never completed. The parity test drives the real CLI end to end and also pins the boundary the plan did not name: the template `default` declares `limits.maxAgentTimeMs`, which direct drive refuses (`run_handoff_required`, `limit_unsupported` — agent-time budget enforcement is not in this slice) — the portal drives a slim agent-only workflow that omits the limit, per the S1 recipe; remaining: ws-api drive/cancel routes and portal controls | S2, S3, exact project binding | command-parity test plus existing duplicate-drive, shutdown and receipt coverage | `portal create-drive-cancel preserves command identity and reports authoritative settlement` in `test/core/studio-layout.test.ts` (landed) |
