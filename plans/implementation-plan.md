...
- **Lean PR gate (2026-09-16, docs-only short circuit 2026-09-23):** CI `Validate` legs run `validate:pr` (core suite, type-check, docs/versions lint, generated-dist currency — no coverage instrumentation, no simulations, no pack) on pull requests; pushes to main run full `validate:ci` (coverage + pack dry-run) and `nightly.yml` keeps the complete-suite 93/80/93 coverage floors. Documentation-only changes run Classify and Docs lint; the required Validate and Plugin jobs preserve their pinned names but execute only explicit no-op skip steps. PR validation builds once before core tests, checks, and the generated-artifact comparison; the workflow-level generated rebuild is non-PR only, and main package inspection is not duplicated. Job names and two-Node matrix remain unchanged (protect-main ruleset pins them). AGENTS.md gate table updated in the same change.
