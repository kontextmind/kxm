...
- **The one queue (replanned 2026-09-20; design record
  ...
  | S5 **(live-run witness passed; edge auth remains)** | Edge authentication and first real use + the deployed restore witness. Done on kxm-dev-svr: box at current main (hub + Runtime as systemd services, S1-conformant paths), `kxm tenant status` live, restore witness passed (manifest + whole-set), and the **one real run** — `run_88ee9faae9d0466a9c9326c66d67e357`, live drive through the Runtime, harness pi → qwen-token-plan/qwen3.8-flash (15.5 s), `resultClass: outcome, passed`, verified receipt. Getting there closed four real gaps as PRs (#264 refused-producer visibility, #265 qualified auth probes, #266 audited pi one-shot with total containment, #262 agent-declared model routes — the pi-one-shot RPC-only decision is reversed and recorded there). **Remaining:** edge HTTPS+Authentik (tenant-admin only) and the write-refusal witness for the contained profile | S4, tenant DNS/TLS/Authentik config | deployed witness: restore ✅, real run ✅; edge auth pending | none — named deployment witness, no new suite |
