...
Extend `vnext-runtime-store.ts` for sequenced extraction events and `vnext-oneshot-process.ts` for bounded live frames while preserving its existing output limits, abort handling, and observed-exit evidence. KXM already has process settlement machinery; its current collector buffers output until completion. [Existing runner](https://github.com/kontextmind/kxm/blob/5ff9f642e82d7a1e52aeab9245282d89560bb535/plugins/kxm/src/vnext-oneshot-process.ts#L49-L145).
