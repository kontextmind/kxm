...
const pluginPackage = await json("plugins/kxm-mesh/package.json");
const pluginManifest = await json("plugins/kxm-mesh/.claude-plugin/plugin.json");
const marketplace = await json(".claude-plugin/marketplace.json");
const mcpSource = await readFile("plugins/kxm-mesh/src/mcp-server.ts", "utf8");
const mcpVersion = mcpSource.match(/const VERSION = "([^"]+)"/)?.[1];
const marketplaceEntry = marketplace.plugins?.find((plugin) => plugin.name === "kxm-mesh");
