...
const mcpSource = await readFile("plugins/pi-mesh-comms/src/mcp-server.ts", "utf8");
const mcpVersion = mcpSource.match(/const VERSION = "([^"]+)"/)?.[1];
const marketplaceEntry = marketplace.plugins?.find((plugin) => plugin.name === "pi-mesh-comms");
