...
Interactive **Pi TUI**: on `startup` / `/new`, the kxm extension offers recent tasks/plans and paints the footer + widget. `/kxm` reopens the picker. `/kxm status` refreshes chrome only. Workers, RPC, and print mode never prompt. `KXM_SESSION_BRIEF=off` disables the picker only.
