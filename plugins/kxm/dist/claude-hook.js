...
var requireFromHere, native, DatabaseSync;
...
var DEFAULT_MESSAGE_TTL_MS, MAX_MESSAGE_TTL_MS, DEFAULT_MESSAGE_RETENTION_MS, MAX_BODY_BYTES, MAX_LEASE_TTL_MS, DEFAULT_LEASE_TTL_MS, IMPROVEMENT_AREAS;
var init_protocol = __esm({
  "plugins/kxm/src/protocol.ts"() {
    "use strict";
    DEFAULT_MESSAGE_TTL_MS = 24 * 60 * 6e4;
    MAX_MESSAGE_TTL_MS = 7 * 24 * 60 * 6e4;
    DEFAULT_MESSAGE_RETENTION_MS = 7 * 24 * 60 * 6e4;
    MAX_BODY_BYTES = 256 * 1024;
    MAX_LEASE_TTL_MS = 10 * 6e4;
    DEFAULT_LEASE_TTL_MS = 5 * 6e4;
    IMPROVEMENT_AREAS = [
      "harness",
      "gates",
      "implementation",
...
__export(local_snapshot_exports, {
  loadLocalMeshSnapshot: () => loadLocalMeshSnapshot,
  resolveKxmSnapshotPaths: () => resolveKxmSnapshotPaths,
  summarizeMeshRun: () => summarizeMeshRun
});
...
function resolveKxmSnapshotPaths(cwd, env = process.env) {
  ...
  const dataPath = configured ? resolve2(cwd, configured) : join(stateDir, "kxm.db");
  return { dataPath, stateDir };
}
function resolveKxmStateRoot(stateDir, options) {
  if (options?.kxmStateRoot && existsSync2(options.kxmStateRoot)) {
    return resolve2(options.kxmStateRoot);
  }
  if (existsSync2(join(stateDir, "runtime", "registry.db")) || existsSync2(join(stateDir, "runtime", "projects"))) {
    return stateDir;
  }
  ...
  if (process.platform === "win32") {
    ...
    base = resolve2(homedir(), "Library", "Application Support", "KXM");
  } else {
...
function loadLocalMeshSnapshot(dataPath, stateDir, options) {
  ...
  if (kxmStateRoot) {
    ...
    if (!scope && existsSync2(projectsDir)) {
      try {
  ...
  if (existsSync2(stateDir)) {
    for (const file of readdirSync(stateDir).filter((name) => name.endsWith(".pid"))) {
      try {
        const record = JSON.parse(readFileSync2(join(stateDir, file), "utf8"));
        pids.push({
          file,
...
var DEFAULT_BUSY_TIMEOUT_MS, RUNTIME_PROJECT_KEY, DONE_STAGE_STATUSES, OPEN_MESSAGE_WHERE, SCOPED_MESSAGE_WHERE;
var init_local_snapshot = __esm({
  "plugins/kxm/src/local-snapshot.ts"() {
    ...
    RUNTIME_PROJECT_KEY = /^[A-Za-z0-9_-][A-Za-z0-9._-]*$/;
    DONE_STAGE_STATUSES = /* @__PURE__ */ new Set(["passed", "failed", "warning"]);
    OPEN_MESSAGE_WHERE = " WHERE json_extract(record, '$.status') IN ('queued', 'delivered')";
    SCOPED_MESSAGE_WHERE = `${OPEN_MESSAGE_WHERE}
...
function readUpdateCache(stateDir, now = Date.now()) {
  const path = join2(stateDir, KXM_UPDATE_CACHE);
...
var KXM_UPDATE_CACHE, CACHE_TTL_MS;
var init_kxm_update = __esm({
  "plugins/kxm/src/kxm-update.ts"() {
    "use strict";
    KXM_UPDATE_CACHE = "update-check.json";
    CACHE_TTL_MS = 6 * 60 * 60 * 1e3;
  }
});

// plugins/kxm/src/hub-binding.ts
import { existsSync as existsSync4, mkdirSync as mkdirSync3, readFileSync as readFileSync4, renameSync, rmSync, writeFileSync as writeFileSync2 } from "node:fs";
...
var session_work_exports = {};
__export(session_work_exports, {
  DEFAULT_SESSION_BRIEF_STALE_SECONDS: () => DEFAULT_SESSION_BRIEF_STALE_SECONDS,
  KXM_SLASH_SUBCOMMANDS: () => KXM_SLASH_SUBCOMMANDS,
  MAX_SESSION_BRIEF_PLANS: () => MAX_SESSION_BRIEF_PLANS,
  MAX_SESSION_BRIEF_TASKS: () => MAX_SESSION_BRIEF_TASKS,
  SESSION_BRIEF_SCHEMA: () => SESSION_BRIEF_SCHEMA,
  SESSION_BRIEF_SKIP_LABEL: () => SESSION_BRIEF_SKIP_LABEL,
  buildSessionBrief: () => buildSessionBrief,
  estimateSessionCost: () => estimateSessionCost,
  formatCostLine: () => formatCostLine,
  formatSessionBriefText: () => formatSessionBriefText,
  formatSessionStatusLine: () => formatSessionStatusLine,
  formatSessionWidget: () => formatSessionWidget,
  formatShipLine: () => formatShipLine,
  itemFromChoice: () => itemFromChoice,
  kxmSlashCompletions: () => kxmSlashCompletions,
  loadSessionBrief: () => loadSessionBrief,
  loadSessionBriefAsync: () => loadSessionBriefAsync,
  parseKxmSlashArgs: () => parseKxmSlashArgs,
  readCachedSessionBrief: () => readCachedSessionBrief,
  readGitShip: () => readGitShip,
  renderStatusLine: () => renderStatusLine,
  resolveSessionHubStatus: () => resolveSessionHubStatus,
  sessionBriefChoices: () => sessionBriefChoices,
  sessionBriefPickerEnabled: () => sessionBriefPickerEnabled,
  writeCachedSessionBrief: () => writeCachedSessionBrief
});
import { spawnSync } from "node:child_process";
import { randomUUID } from "node:crypto";
import { existsSync as existsSync5, mkdirSync as mkdirSync4, readFileSync as readFileSync5, renameSync as renameSync2, writeFileSync as writeFileSync3 } from "node:fs";
...
function formatCostLine(input) {
  if (!input) return void 0;
  if (input.sessionCostUsd === void 0 && input.runCostUsd === void 0) return "unknown";
  const sess = input.sessionCostUsd !== void 0 ? `$${input.sessionCostUsd.toFixed(2)} sess` : void 0;
  const run = input.runCostUsd !== void 0 ? `$${input.runCostUsd.toFixed(2)} run` : void 0;
  const route = input.harness && input.model ? `${input.harness}/${input.model}` : input.model ?? void 0;
  const parts = [sess, run, route].filter(Boolean);
  return parts.length > 0 ? parts.join(" \xB7 ") : "unknown";
}
function renderStatusLine(statsOrBrief, current, hub, ship, updateLatest, cost) {
  ...
  return formatSessionStatusLine(statsOrBrief, current, hub, ship, updateLatest, cost);
}
...
function readCachedSessionBrief(stateDir) {
  const file = join4(stateDir, "session-brief.json");
  try {
...
var SESSION_BRIEF_SKIP_LABEL, MAX_SESSION_BRIEF_TASKS, MAX_SESSION_BRIEF_PLANS, SESSION_BRIEF_SCHEMA, DEFAULT_SESSION_BRIEF_STALE_SECONDS, KXM_SLASH_SUBCOMMANDS;
var init_session_work = __esm({
  "plugins/kxm/src/session-work.ts"() {
    "use strict";
    init_local_snapshot();
    init_kxm_update();
    init_hub_binding();
    init_telemetry();
    SESSION_BRIEF_SKIP_LABEL = "Skip \u2014 start a fresh session";
    MAX_SESSION_BRIEF_TASKS = 5;
    MAX_SESSION_BRIEF_PLANS = 5;
    SESSION_BRIEF_SCHEMA = "kxm.session-brief.v1";
    DEFAULT_SESSION_BRIEF_STALE_SECONDS = 5;
    KXM_SLASH_SUBCOMMANDS = ["status", "hub", "memory", "progress", "workflow", "help"];
  }
});
...
__export(memory_exports, {
  MEMORY_BRIEF_SCHEMA: () => MEMORY_BRIEF_SCHEMA,
  MEMORY_MARKER_END: () => MEMORY_MARKER_END,
  MEMORY_MARKER_START: () => MEMORY_MARKER_START,
  MEMORY_SCHEMA: () => MEMORY_SCHEMA,
  createMemoryNote: () => createMemoryNote,
  formatHarnessMemoryBlock: () => formatHarnessMemoryBlock,
  formatMemoryBriefText: () => formatMemoryBriefText,
  formatMemoryRecord: () => formatMemoryRecord,
  generateMemoryBrief: () => generateMemoryBrief,
  loadAuthoredMemory: () => loadAuthoredMemory,
...
import { extname, join as join5, resolve as resolve4 } from "node:path";
...
function memoryDirectories(repoRoot) {
  ...
  const memoryDir = join5(root, ".kxm", "memory");
  const candidatesDir = join5(memoryDir, "candidates");
  return { memoryDir, candidatesDir };
}
function loadAuthoredMemory(repoRoot) {
  const { memoryDir } = memoryDirectories(repoRoot);
  if (!existsSync6(memoryDir)) return [];
  const records = [];
  const entries = readdirSync2(memoryDir, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isFile() && extname(entry.name) === ".md") {
      const fullPath = join5(memoryDir, entry.name);
      try {
        const text = readFileSync6(fullPath, "utf8");
...
function loadCandidateMemory(repoRoot) {
  const { candidatesDir } = memoryDirectories(repoRoot);
  if (!existsSync6(candidatesDir)) return [];
  const records = [];
  const entries = readdirSync2(candidatesDir, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isFile() && extname(entry.name) === ".md") {
      const fullPath = join5(candidatesDir, entry.name);
      try {
        const text = readFileSync6(fullPath, "utf8");
        const record = parseMemoryRecord(text, entry.name);
        records.push(record);
...
function createMemoryNote(repoRoot, summary, options = {}) {
  ...
  const targetPath = join5(candidatesDir, `${id}.md`);
  if (options.dryRun) return { record, path: targetPath };
  mkdirSync5(candidatesDir, { recursive: true });
  writeFileSync4(targetPath, formatMemoryRecord(record), "utf8");
...
var init_memory = __esm({
  "plugins/kxm/src/memory.ts"() {
    ...
    MEMORY_MARKER_START = "<!-- kxm:memory:start -->";
    MEMORY_MARKER_END = "<!-- kxm:memory:end -->";
    VALID_SCOPES = /* @__PURE__ */ new Set(["agent", "project", "run", "operator"]);
    VALID_AUTHORITIES = /* @__PURE__ */ new Set(["instruction", "evidence", "promoted"]);
    VALID_CONFIDENCES = /* @__PURE__ */ new Set(["verified", "probable", "uncertain"]);
    VALID_LIFECYCLES = /* @__PURE__ */ new Set(["active", "deprecated", "superseded"]);
    BANNED_CONTROL_PLANE_FIELDS = /* @__PURE__ */ new Set([
      "permissions",
      "tools",
      "allow",
      "deny",
...
var RELEVANCE_STOPWORDS;
var init_relevance = __esm({
  "plugins/kxm/src/relevance.ts"() {
    "use strict";
    RELEVANCE_STOPWORDS = Object.freeze(/* @__PURE__ */ new Set([
      "a",
      "an",
      "and",
      "are",
...
var init_client = __esm({
  "plugins/kxm/src/client.ts"() {
    ...
    init_protocol();
    init_workflow();
    HubHttpError = class extends Error {
      statusCode;
      code;
      requestId;
      extras;
...
__export(commands_exports, {
  ...
  isSessionTokenExpired: () => isSessionTokenExpired,
  isToolAllowed: () => isToolAllowed,
  mintAttemptToken: () => mintAttemptToken,
  mintSessionToken: () => mintSessionToken,
  parseAttemptToken: () => parseAttemptToken,
  parseSessionToken: () => parseSessionToken,
  persistSessionTokenToDisk: () => persistSessionTokenToDisk,
  readSessionTokenFromDisk: () => readSessionTokenFromDisk,
  reconcileInbox: () => reconcileInbox,
...
function mintAttemptToken(input) {
  ...
  const payload = {
    ...
    assignmentId: input.assignmentId ?? `assign-${randomUUID3()}`,
    attemptId: input.attemptId ?? `attempt-${randomUUID3()}`,
...
function mintSessionToken(input) {
  ...
  const payload = {
    ...
    sessionId: input?.sessionId ?? `session-${randomUUID3()}`,
    issuedAt,
    expiresAt,
...
var AGENT_COMMANDS, AGENT_COMMANDS_MAP;
...
// plugins/kxm/src/claude-hook.ts
var STDIN_LIMIT_BYTES = 64 * 1024;
var STDIN_WAIT_MS = 300;
var HUB_PROBE_MS = 300;
var SNAPSHOT_BUSY_TIMEOUT_MS = 250;
var STATUS_MAX_CHARS = 1500;
var MAX_ACTIVE_RUNS = 3;
var ACTIVE_RUN_STATUSES = /* @__PURE__ */ new Set(["running", "waiting", "created", "preparing"]);
var DEFAULT_SERVER_URL = "http://127.0.0.1:7331";
  ...
  return new Promise((resolveInput) => {
    ...
    const finish = (overflow) => {
      ...
      try {
        ...
        resolveInput(parsed && typeof parsed === "object" && !Array.isArray(parsed) ? parsed : {});
      } catch {
        resolveInput({});
      }
    };
...
async function sessionStartContext(input, env) {
  ...
  const [
    ...
    Promise.resolve().then(() => (init_memory(), memory_exports)),
    Promise.resolve().then(() => (init_project_name(), project_name_exports)),
    Promise.resolve().then(() => (init_commands(), commands_exports)),
    Promise.resolve().then(() => __toESM(require_dist(), 1))
  ]);
  const hubProject = defaultProjectName2(root, { ...env, KXM_PROJECT: env.CLAUDE_PLUGIN_OPTION_PROJECT ?? "" });
  const recipientName = env.CLAUDE_PLUGIN_OPTION_AGENT_NAME?.trim() || void 0;
  ...
  const hub = await resolveSessionHubStatus2(url, fetch, HUB_PROBE_MS);
  const lines = [`KXM project ${oneLine(hubProject)} \xB7 hub ${hub.state} at ${url}`];
  try {
    const paths = resolveKxmSnapshotPaths2(root, env);
    const snapshot = loadLocalMeshSnapshot2(paths.dataPath, paths.stateDir, {
      env,
      projectRoot: root,
  ...
  const tokenHint = sessionTokenFixHint(enforceToolPolicy2("kxm_list", env));
  if (tokenHint) lines.push(tokenHint);
  const status = truncate2(lines.join("\n"), STATUS_MAX_CHARS);
  let memory;
  try {
    memory = formatMemoryBriefText2(generateMemoryBrief2(root));
  } catch {
  }
  return memory === void 0 ? status : `${status}
