...
export interface VnextCommandDefinition {
...
export interface VnextCommandGateSeams {
  timing?: { termGraceMs?: number; finalWaitMs?: number; lingerMs?: number } | undefined;
  beforeSpawn?: ((definition: VnextCommandDefinition) => void) | undefined;
  afterSpawned?: ((pid: number) => void) | undefined;
  afterStopRequested?: ((cause: VnextGateStopCause) => void) | undefined;
  afterChildClose?: (() => void) | undefined;
}

export const vnextCommandGateSeams: VnextCommandGateSeams = {};

export type VnextCommandOutcome =
  | { kind: "no-start"; observation: VnextGateObservationInput }
  | { kind: "complete"; observation: VnextGateObservationInput }
  | { kind: "uncertain"; reason: string; observation: VnextGateObservationInput };
 ...
 */
export const POST_SPAWN_CHILD_ERROR_CLASS: VnextGateErrorClass = "stream-error";

export interface VnextCommandObserver {
  requestStop(cause: VnextGateStopCause, errorClass?: VnextGateErrorClass): void;
  beginBoundedCleanup(): void;
  run(): Promise<VnextCommandOutcome>;
}

export function createCommandObserver(input: {
  definition: VnextCommandDefinition;
  cwd: string;
  signal: AbortSignal;
  onSpawned: (pid: number) => void;
}): VnextCommandObserver {
...
class CommandObserver implements VnextCommandObserver {
  private readonly definition: VnextCommandDefinition;
  ...
  private stopCause: VnextGateStopCause = "none";
  private signalsAttempted: "none" | "term" | "term-kill" = "none";
  private errorClass: VnextGateErrorClass | null = null;
  ...
  private resolveOutcome: ((outcome: VnextCommandOutcome) => void) | undefined;
  ...
  private uncertaintyReason(observation: VnextGateObservationInput): string {
  ...
  private snapshot(): VnextGateObservationInput {
  ...
  private baseObservation(
    facts: Omit<VnextGateObservationInput, "checkedCount" | "failedCount" | "elapsedMs" | "startedAt"> & {
      elapsedMs?: number;
      startedAt?: string;
    },
  ): VnextGateObservationInput {
  ...
  private handoffWith(outcome: VnextCommandOutcome): void {
    if (this.handedOff) return;
    this.handedOff = true;
    this.canSignal = false;
...
function dropVnextCommandStopCapability(observer: CommandObserver): void {
  observer.requestStop = () => undefined;
}

function isCompleteNormal(observation: VnextGateObservationInput): boolean {
