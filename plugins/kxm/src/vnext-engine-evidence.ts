...
export function verifyVnextGateEvidence(
  store: VnextRunEventStore,
  run: VnextRunRecord,
...
function lastEffectByAttempt(events: readonly VnextRunEvent[]): Map<string, string> {
...
function assertShared(run: VnextRunRecord, row: VnextGateObservationRow | VnextGateEvidenceRow, attempt: VnextGateAttemptRow): void {
  if (
...
function stringField(event: VnextRunEvent, field: string): string {
