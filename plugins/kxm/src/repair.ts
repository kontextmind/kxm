...
import {
  ...
  renderKxmTemplate,
  resolveKxmTemplateBaseline,
  SUPPORTED_KXM_TEMPLATE_VARIANTS,
  kxmContentSha256,
  type KxmRenderedTemplate,
  type KxmTemplateProvenance,
  type KxmTemplateVariant,
} from "./template.ts";
import { computeKxmResourcePermissionDiff } from "./permission.ts";
import type { KxmResourceKind } from "./project-config.ts";

/** Map a managed template path to its resource kind for permission diffing. */
function resourceKindForTemplatePath(path: string): KxmResourceKind {
...
export type KxmTemplateClassification = "unchanged" | "user-only" | "template-only" | "converged" | "conflict";

export interface KxmTemplateChange {
  path: string;
  classification: KxmTemplateClassification;
  baseSha256?: string;
  localSha256?: string;
  targetSha256?: string;
...
export interface KxmTemplateRepairPlan {
  projectRoot: string;
  projectId: string;
  projectName: string;
  ...
  canApply: boolean;
  changes: readonly KxmTemplateChange[];
  issues: readonly KxmConfigIssue[];
}

interface KxmInitOperationFile {
  path: string;
...
interface KxmInitOperation {
  ...
  targetTemplateRevision: string;
  planSha256: string;
  files: readonly KxmInitOperationFile[];
}

export interface KxmOperationResult {
  kind: "create" | "repair";
  resumed: boolean;
...
export interface KxmRepairRuntimeOptions {
  schemasDir?: string;
...
function repairIssue(code: string, file: string, message: string, phase: KxmConfigIssue["phase"] = "semantic"): KxmConfigIssue {
  return { phase, code, file, message };
}

function fail(code: string, file: string, message: string, phase: KxmConfigIssue["phase"] = "semantic"): never {
...
export function kxmInitTransactionPath(projectRoot: string): string {
...
function readProjectAndProvenance(projectRoot: string, schemasDir?: string): { project: JsonObject; provenance: KxmTemplateProvenance; provenanceBytes: Buffer } | undefined {
  const projectFile = join(projectRoot, ".kxm", "project.yaml");
  const provenanceFile = join(projectRoot, ...KXM_TEMPLATE_PROVENANCE_PATH.split("/"));
  ...
  const provenanceValue = parseRestrictedYaml(provenanceBytes, KXM_TEMPLATE_PROVENANCE_PATH);
  ...
  if (!provenance) fail("template_provenance_revision_invalid", KXM_TEMPLATE_PROVENANCE_PATH, "template provenance does not exactly match a supported built-in baseline");
  if (project.id !== provenance.inputs.projectId) {
    fail("template_provenance_project_mismatch", KXM_TEMPLATE_PROVENANCE_PATH, "template provenance belongs to a different project identity");
  }
  const seen = new Set<string>();
  let prior = "";
  let totalBytes = 0;
  for (const record of provenance.files) {
    if (!portableManagedPath(record.path)) fail("template_provenance_path_invalid", KXM_TEMPLATE_PROVENANCE_PATH, `managed path ${record.path} is invalid`, "path");
    const folded = record.path.toLocaleLowerCase("en-US");
    if (seen.has(folded)) fail("template_provenance_path_collision", KXM_TEMPLATE_PROVENANCE_PATH, `managed path ${record.path} collides after case folding`, "path");
    if (prior && compareCodeUnits(prior, record.path) >= 0) fail("template_provenance_order_invalid", KXM_TEMPLATE_PROVENANCE_PATH, "managed files are not in strict code-unit order");
    totalBytes += record.bytes;
    seen.add(folded);
    prior = record.path;
  }
  if (provenance.files.length > MAX_MANAGED_FILES || totalBytes > MAX_MANAGED_TOTAL_BYTES) {
    fail("template_provenance_bounds_exceeded", KXM_TEMPLATE_PROVENANCE_PATH, "managed template manifest exceeds bounded file or byte limits", "parse");
  }
...
export function planKxmTemplateRepair(
  projectRoot: string,
  options: KxmRepairRuntimeOptions = {},
): KxmTemplateRepairPlan | undefined {
  ...
  const sourceRenderer = SUPPORTED_KXM_TEMPLATE_VARIANTS
    .map((variant) => renderKxmTemplate(source.provenance.inputs.projectId, source.provenance.inputs.projectName, variant))
  ...
  const changes = paths.map((path): KxmTemplateChange => {
    ...
    if (base && localSha256 === base.sha256 && local?.byteLength !== base.bytes) {
      fail("template_provenance_bytes_invalid", KXM_TEMPLATE_PROVENANCE_PATH, `recorded byte count for ${path} does not match its baseline content`);
    }
    ...
    if (localSha256 === baseSha256 && targetRecord) {
      ...
      if (authorityChanged || !base) {
        ...
        const detail = baseValue && targetValue
          ? computeKxmResourcePermissionDiff(resourceKindForTemplatePath(path), path, baseValue, targetValue)
...
function operationPlanSha(operation: Omit<KxmInitOperation, "planSha256" | "phase">): string {
...
function writeOperation(projectRoot: string, operation: KxmInitOperation): void {
...
function updatePhase(projectRoot: string, operation: KxmInitOperation, phase: KxmInitOperation["phase"]): KxmInitOperation {
  const updated = { ...operation, phase };
  writeOperation(projectRoot, updated);
  return updated;
}

function readOperation(projectRoot: string, schemasDir?: string): KxmInitOperation | undefined {
  const root = transactionRoot(projectRoot);
  ...
  if (issues.length > 0) throw new KxmConfigError(issues);
  const operation = value as unknown as KxmInitOperation;
...
export function hasKxmInitTransaction(projectRoot: string): boolean {
...
function writeAndVerifyBackups(projectRoot: string, operation: KxmInitOperation): void {
...
function writeKnownBackups(projectRoot: string, operation: KxmInitOperation): void {
...
function verifyBackups(projectRoot: string, operation: KxmInitOperation): void {
  if (operation.kind !== "repair") return;
  for (const entry of operation.files) {
    if (entry.observedSha256 === null) continue;
...
function verifyTargetArtifacts(projectRoot: string, operation: KxmInitOperation): void {
  let total = 0;
  for (const entry of operation.files) {
    ...
    if (kxmContentSha256(bytes) !== entry.targetSha256) fail("init_transaction_target_mismatch", entry.path, "pinned target artifact hash does not match the operation");
  }
...
function loaderOptions(options: KxmRepairRuntimeOptions): KxmConfigOptions {
  assertNoRegisteredGates(options);
  return {
...
function prepareShadow(projectRoot: string, operation: KxmInitOperation, options: KxmRepairRuntimeOptions): void {
  ...
  if (!provenance || provenance.templateRevision !== operation.targetTemplateRevision) {
    fail("repair_shadow_provenance_invalid", KXM_TEMPLATE_PROVENANCE_PATH, "prospective template provenance does not match the pinned target");
  }
}
...
function createOperation(
  ...
  const operationWithoutHash = {
    ...
    templateId: KXM_TEMPLATE_ID as typeof KXM_TEMPLATE_ID,
...
function validateOperationRecord(operation: KxmInitOperation, schemasDir?: string): void {
...
function prepareOperation(
  ...
  try {
    for (const entry of operation.files) {
      ...
      if (!bytes || kxmContentSha256(bytes) !== entry.targetSha256) fail("template_target_unavailable", entry.path, "renderer cannot reproduce the pinned target bytes");
...
function destinationSha(projectRoot: string, path: string): string | undefined {
  const file = join(projectRoot, ...path.split("/"));
  if (!existsSync(file)) return undefined;
  return kxmContentSha256(readRegularBounded(file, path));
}
...
function atomicInstallTarget(projectRoot: string, operation: KxmInitOperation, entry: KxmInitOperationFile): void {
  ...
  if (existsSync(displaced)) {
    ...
    if (entry.observedSha256 === null || kxmContentSha256(readFileSync(displaced)) !== entry.observedSha256) {
  ...
  try {
    ...
    if (entry.observedSha256 !== null) {
      ...
      syncDirectory(directory);
      if (kxmContentSha256(readFileSync(displaced)) !== entry.observedSha256) {
        if (!existsSync(destination)) {
          try { linkSync(displaced, destination); }
...
function installedCreateMatches(projectRoot: string, operation: KxmInitOperation): boolean {
...
function rebuildCreateShadow(projectRoot: string, operation: KxmInitOperation, options: KxmRepairRuntimeOptions): void {
  const shadow = shadowRoot(projectRoot);
...
function applyOperation(
  ...
  if (!source || source.provenance.templateRevision !== operation.targetTemplateRevision) {
    fail("init_operation_verification_failed", KXM_TEMPLATE_PROVENANCE_PATH, "installed provenance does not match the pinned transaction");
  }
...
export function prepareAndApplyKxmCreate(
  projectRoot: string,
  rendered: KxmRenderedTemplate,
  options: KxmRepairRuntimeOptions = {},
): KxmOperationResult {
  assertNoRegisteredGates(options);
  const files = [...rendered.files.entries()].map(([path, bytes]): KxmInitOperationFile => ({
    path,
    observedSha256: null,
...
export function prepareAndApplyKxmRepair(
  plan: KxmTemplateRepairPlan,
  options: KxmRepairRuntimeOptions = {},
): KxmOperationResult {
  assertNoRegisteredGates(options);
  ...
  if (!provenanceBytes) fail("template_provenance_target_missing", KXM_TEMPLATE_PROVENANCE_PATH, "renderer did not produce provenance");
  const sourceProvenance = readOptionalManaged(plan.projectRoot, KXM_TEMPLATE_PROVENANCE_PATH);
  if (!sourceProvenance) fail("template_provenance_missing", KXM_TEMPLATE_PROVENANCE_PATH, "repair requires recorded provenance");
  const files: KxmInitOperationFile[] = actionable.map((change) => ({
    path: change.path,
...
function rendererForRevision(operation: KxmInitOperation, revision: string): KxmRenderedTemplate | undefined {
...
function rendererForOperation(operation: KxmInitOperation): KxmRenderedTemplate | undefined {
...
function validateOperationIntent(projectRoot: string, operation: KxmInitOperation): void {
  ...
  if (operation.kind === "create") {
    ...
    for (const [path, bytes] of target.files) {
      ...
      if (!entry
        ...
        || entry.targetSha256 !== kxmContentSha256(bytes)
  ...
  const sourceProvenanceBytes = source.files.get(KXM_TEMPLATE_PROVENANCE_PATH) as Buffer;
  const targetProvenanceBytes = target.files.get(KXM_TEMPLATE_PROVENANCE_PATH) as Buffer;
  if (!provenanceEntry
    || provenanceEntry.observedSha256 !== kxmContentSha256(sourceProvenanceBytes)
    || provenanceEntry.targetSha256 !== kxmContentSha256(targetProvenanceBytes)
    || provenanceEntry.classification !== "template-only"
    || provenanceEntry.action !== "replace") {
    fail("init_transaction_provenance_intent_invalid", KXM_TEMPLATE_PROVENANCE_PATH, "repair provenance action does not match supported source and target templates");
  }
  for (const [path, entry] of entries) {
    if (path === KXM_TEMPLATE_PROVENANCE_PATH) continue;
...
export function resumeKxmInitTransaction(
  projectRoot: string,
  options: KxmRepairRuntimeOptions = {},
): KxmOperationResult | undefined {
  assertNoRegisteredGates(options);
...
export function commitKxmInitTransaction(projectRoot: string, schemasDir?: string): void {
...
export function inspectKxmInitTransaction(projectRoot: string, schemasDir?: string): KxmInitOperation | undefined {
  const operation = readOperation(projectRoot, schemasDir);
