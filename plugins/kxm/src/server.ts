...
const host = process.env.KXM_HOST ?? "127.0.0.1";
const port = Number.parseInt(process.env.KXM_PORT ?? String(DEFAULT_PORT), 10);
const authToken = process.env.KXM_AUTH_TOKEN;
const workspaceDir = resolve(process.env.KXM_WORKSPACE_DIR?.trim() || ".kxm");
const configDir = resolve(process.env.KXM_CONFIG_DIR?.trim() || join(workspaceDir, "config"));
const logsDir = resolve(process.env.KXM_LOGS_DIR?.trim() || join(workspaceDir, "logs"));
const assetsDir = resolve(process.env.KXM_ASSETS_DIR?.trim() || join(workspaceDir, "assets"));
const stateDir = resolve(process.env.KXM_STATE_DIR?.trim() || join(workspaceDir, "state"));
const dataPathValue = process.env.KXM_DATA_PATH?.trim();
const dataPath = dataPathValue === ":memory:" ? dataPathValue : resolve(dataPathValue || join(stateDir, "kxm.db"));
const logPath = resolve(process.env.KXM_LOG_PATH?.trim() || join(logsDir, "kxm-hub.jsonl"));
const messageTtlMs = Number.parseInt(process.env.KXM_MESSAGE_TTL_MS ?? String(DEFAULT_MESSAGE_TTL_MS), 10);
...
const inlineWorkflows = process.env.KXM_WEBHOOK_WORKFLOWS?.trim();
const workflowFile = process.env.KXM_WEBHOOK_WORKFLOWS_FILE?.trim();
