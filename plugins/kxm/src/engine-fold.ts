import type { KxmCompiledPlan, KxmCompiledStep, KxmCompiledTransition, KxmTerminalStatus } from "./engine-compile.ts";
import {
  runtimeError,
  type KxmRunEvent,
  type KxmRunRecord,
...
export const KXM_RUN_STATE_SCHEMA = "kxm.run-state.v2";
export const FOLD_PANEL_BOUND = 1;

const RUN_STATUSES = new Set<KxmRunStatus>(["created", "preparing", "running", "waiting", "blocked_uncertain", "cancelling", "cancelled", "completed", "failed"]);
const TERMINAL_RUN = new Set<KxmRunStatus>(["cancelled", "completed", "failed"]);
...
const SLICE_BLOCKED_RUN = new Set<KxmRunStatus>(["waiting"]);
const RUN_EDGES: Readonly<Record<string, ReadonlySet<KxmRunStatus>>> = {
...
export interface KxmFoldOptions {
...
export type KxmRunDurationBudgetSource = "workflow" | "project" | "both";

export interface KxmRunDurationBudget {
  readonly budgetMs: number;
  readonly source: KxmRunDurationBudgetSource;
}
...
): KxmRunDurationBudget | undefined {
...
export interface KxmFoldAttemptState {
  readonly status: string;
  readonly resultClass?: string | undefined;
  readonly outcome?: string | undefined;
}

export interface KxmFoldAssignmentState {
  readonly status: string;
  readonly currentAttemptId?: string | undefined;
  readonly attempts: Readonly<Record<string, KxmFoldAttemptState>>;
}

export interface KxmFoldPanel {
  readonly order: readonly string[];
  readonly assignments: Readonly<Record<string, KxmFoldAssignmentState>>;
}

export type KxmJoinAllResult =
...
export interface KxmRunCurrentStep {
  readonly stepId: string;
  readonly stepAttempt: number;
  readonly status: string;
  readonly panel: KxmFoldPanel;
  ...
  readonly effectState?: KxmRunEffectState | undefined;
...
export interface KxmRunDriveBinding {
  readonly driveId: string;
  readonly mode: "simulated" | "live";
  readonly openedSequence: number;
}

export interface KxmRunState {
  readonly schema: typeof KXM_RUN_STATE_SCHEMA;
  readonly runId: string;
  readonly status: KxmRunStatus;
  readonly runPlanHash?: string | undefined;
  readonly pendingStepId?: string | undefined;
  readonly currentStep?: KxmRunCurrentStep | undefined;
  ...
  readonly drive?: KxmRunDriveBinding | undefined;
...
interface MutableCurrentStep {
  ...
  effectState?: KxmRunEffectState | undefined;
...
interface MutableState {
  schema: typeof KXM_RUN_STATE_SCHEMA;
  runId: string;
  status: KxmRunStatus;
  ...
  lastTerminalTransition?: KxmTerminalStatus | undefined;
  drive?: KxmRunDriveBinding | undefined;
...
export function kxmFoldPanelAttempt(
  step: KxmRunCurrentStep | MutableCurrentStep | undefined,
  attemptId: string,
): { assignmentId: string; assignment: MutableAssignment | KxmFoldAssignmentState; attempt: MutableAttempt | KxmFoldAttemptState } | undefined {
...
function panelAssignmentBound(plan: KxmCompiledPlan | undefined, current: MutableCurrentStep): number {
...
function createdOnlySingletonCancel(state: MutableState, step: KxmCompiledStep, current: MutableCurrentStep, status: string): boolean {
...
function authorizedCancelStep(state: MutableState, step: KxmCompiledStep, current: MutableCurrentStep, status: string): boolean {
...
function expectedStatusForDeclaredOutcome(step: KxmCompiledStep, outcome: string): "passed" | "failed" | "cancelled" {
...
export function kxmJoinAll(step: KxmCompiledStep, panel: KxmFoldPanel | MutablePanel): KxmJoinAllResult {
...
export function kxmFoldPanelAttemptIds(step: KxmRunCurrentStep | MutableCurrentStep | undefined): string[] {
  if (!step) return [];
  const ids: string[] = [];
  for (const assignmentId of step.panel.order) {
    const assignment = step.panel.assignments[assignmentId];
...
export function foldKxmRunState(
  run: KxmRunRecord,
  plan: KxmCompiledPlan | undefined,
  events: readonly KxmRunEvent[],
  options: KxmFoldOptions = {},
): KxmRunState {
  if (events.length === 0) {
...
function assertEventIdentity(run: KxmRunRecord, event: KxmRunEvent): void {
...
function foldRunCreated(state: MutableState, run: KxmRunRecord, event: KxmRunEvent, index: number): void {
...
function foldRunStatus(state: MutableState, plan: KxmCompiledPlan | undefined, event: KxmRunEvent): void {
  const next = event.payload.status;
  if (typeof next !== "string" || !RUN_STATUSES.has(next as KxmRunStatus)) {
    throw runtimeError("run_events_illegal", state.runId, `run.status_changed has invalid status ${String(next)}`);
  }
  const status = next as KxmRunStatus;
...
function isProvenFailure(state: MutableState, plan: KxmCompiledPlan | undefined): boolean {
...
function foldStepEntered(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): void {
...
function foldStepStatus(state: MutableState, plan: KxmCompiledPlan | undefined, event: KxmRunEvent): void {
...
function foldOutcome(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): void {
...
function foldTransitioned(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): void {
...
function assertTransitionPayload(runId: string, selected: KxmCompiledTransition, event: KxmRunEvent): void {
  if (selected.to === "step") {
...
function foldAssignmentCreated(state: MutableState, plan: KxmCompiledPlan | undefined, event: KxmRunEvent): void {
...
function foldAssignmentAdvance(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent, next: string): void {
...
function foldAttemptCreated(state: MutableState, event: KxmRunEvent): void {
...
function foldAttemptStatus(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): void {
...
function requireGateEffect(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): NonNullable<MutableState["currentStep"]> {
...
function assertEffectIdentity(state: MutableState, event: KxmRunEvent, current: NonNullable<MutableState["currentStep"]>): void {
...
function foldEffectIntent(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): void {
...
function foldEffectDispatched(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): void {
  const current = requireGateEffect(state, plan, event);
...
function foldEffectObserved(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent, options: KxmFoldOptions): void {
  const current = requireGateEffect(state, plan, event);
  ...
  let effectState: KxmRunEffectState = "observed-unknown";
  if (looked) {
...
function foldEffectSettled(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): void {
  const current = requireGateEffect(state, plan, event);
...
function foldEffectUncertain(state: MutableState, plan: KxmCompiledPlan, event: KxmRunEvent): void {
  const current = requireGateEffect(state, plan, event);
  if (current.effectState === "observed-complete" || current.effectState === "observed-no-start" || current.effectState === "observed-unknown") {
...
function requireStep(plan: KxmCompiledPlan, stepId: string, runId: string): KxmCompiledStep {
  const step = plan.steps[stepId];
  if (!step) throw runtimeError("run_events_illegal", runId, `unknown step ${stepId}`);
  return step;
}

function stringPayload(event: KxmRunEvent, field: string): string {
...
function integerPayload(event: KxmRunEvent, field: string): number {
  const value = event.payload[field];
...
function freezePanel(panel: MutablePanel): KxmFoldPanel {
  const assignments: Record<string, KxmFoldAssignmentState> = Object.create(null);
  for (const assignmentId of Object.keys(panel.assignments).sort()) {
    const assignment = panel.assignments[assignmentId]!;
    const attempts: Record<string, KxmFoldAttemptState> = Object.create(null);
...
function freezeCurrentStep(current: MutableCurrentStep): KxmRunCurrentStep {
...
function freezeState(state: MutableState): KxmRunState {
  const frozen: KxmRunState = {
    schema: KXM_RUN_STATE_SCHEMA,
    runId: state.runId,
    status: state.status,
...
export function isTerminalRunStatus(status: KxmRunStatus): boolean {
  return TERMINAL_RUN.has(status);
}

export function isKxmTerminalStatus(value: string): value is KxmTerminalStatus {
