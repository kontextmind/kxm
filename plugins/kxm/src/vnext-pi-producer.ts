...
export interface VnextPiProducerOptions {
...
export interface VnextPiProducer extends VnextProducer {
  readonly id: "pi";
...
export function createVnextPiProducer(options: VnextPiProducerOptions = {}): VnextPiProducer {
  ...
  function resolveModelForRequest(request: VnextProducerRequest): { provider: string; model: string; thinking?: string | undefined } {
    if (request.model) {
  ...
  async function getOrCreateSession(request: VnextProducerRequest, model: string): Promise<PiSession> {
  ...
  const producer: VnextPiProducer = {
    id: "pi",
