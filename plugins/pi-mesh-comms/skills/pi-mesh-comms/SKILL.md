...
4. Call `mesh_workflow_checkpoint` with the active stage, result, summary, and an evidence object keyed by every exact `requiredEvidence` identity. Extra evidence does not replace a named requirement. Correct and repeat any warning or failure; its diagnostic evidence is journaled but does not count toward the later passing attempt.
5. For a mixture-of-agents planning stage, use `mesh_fanout` with one to three capable peers. In a durable workflow, pass the workflow run ID as `correlationId` and a stable, stage-specific `idempotencyKeyPrefix` such as `planning-v1`. Normally omit message TTL for model work. A local wait ending returns a recoverable pending handle; use `mesh_get` or repeat the exact fanout with the same correlation and prefix, never a new idempotency key. Pending peers have not supplied evidence and cannot count toward a multi-agent checkpoint. Collect completed independent plans before showing agents one another's answers, record disagreements, then synthesize the strongest compatible recommendations.
6. When an external system must finish asynchronously, call `mesh_workflow_wait` with the active stage, a stable signal key, expected result, bounded timeout, and any verified local evidence keyed by requirement. The later passing callback accumulates with that evidence and must supply every remaining named requirement. After the hub reports `waiting`, settle the turn. Do not fabricate a callback result or keep the turn open merely to poll.
