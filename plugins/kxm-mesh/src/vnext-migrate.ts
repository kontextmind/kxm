...
import {
  ...
  parseRestrictedYaml,
  readVnextMigrationReceipt,
  validateVnextResources,
  verifyVnextMigrationReceiptTarget,
  vnextCanonicalJson,
  VNEXT_MIGRATION_RECEIPT_PATH,
  type JsonObject,
  type JsonValue,
...
import { withVnextLocalBindingLock, type VnextLocalBindingLock, type VnextLocalBindingStoreOptions } from "./vnext-bindings.ts";
...
function migrateIssue(phase: VnextConfigIssue["phase"], code: string, file: string, message: string): VnextConfigIssue {
...
export interface VnextLegacySource {
  path: string;
  sha256: string;
  bytes: number;
  value: JsonValue;
...
export function readVnextLegacySources(root: string): VnextLegacySource[] {
  ...
  return files.map((path) => {
    ...
    if (stat.isSymbolicLink() || !stat.isFile()) {
      throw new VnextConfigError([migrateIssue("path", "legacy_source_invalid", path, "legacy configuration must be a regular file, not a link")]);
    }
    ...
    try {
      ...
      throw new VnextConfigError([migrateIssue("parse", "invalid_utf8", path, "document is not valid UTF-8")]);
    }
    return {
      path,
...
export function legacySourceDigest(sources: readonly VnextLegacySource[]): string {
...
export interface VnextMigrationAmbiguity {
  key: string;
...
export interface VnextMigrationUnmapped {
  sourcePath: string;
  jsonPointer: string;
  valueSha256: string;
  sensitive: boolean;
...
export interface VnextMigrationRename {
  from: string;
  to: string;
  kind: "agent" | "model" | "workflow" | "step" | "evidence" | "outcome";
}

export interface VnextMigrationPermissionChange {
  sourcePath: string;
  targetPath: string;
...
interface MutablePlan {
  ambiguities: VnextMigrationAmbiguity[];
  invalidDecisions: string[];
  decisionKeys: Set<string>;
  decisionAllowedValues: Map<string, readonly (string | number)[]>;
  unmapped: VnextMigrationUnmapped[];
  renames: VnextMigrationRename[];
  permissionChanges: VnextMigrationPermissionChange[];
}
...
function recordUnmapped(plan: MutablePlan, sourcePath: string, pointer: string, value: JsonValue | undefined, sensitive: boolean): void {
  ...
  const digestInput = sensitive
    ? { redacted: true, bytes: Buffer.byteLength(vnextCanonicalJson(value === undefined ? null : value), "utf8") }
...
function mapIdentity(
  ...
  sourcePath: string,
  raw: string,
  kind: VnextMigrationRename["kind"],
...
function resolveDecision(
  plan: MutablePlan,
  key: string,
  category: VnextMigrationAmbiguity["category"],
  sourcePath: string,
  message: string,
  allowedValues: readonly (string | number)[],
  decisions: Readonly<Record<string, JsonValue>>,
...
function convertAgentsRoster(
  ...
  for (const [index, entry] of list.entries()) {
    ...
    if (thinking) {
      // vNext agents select models; thinking belongs to a model profile. Emit
...
function convertTransition(
  ...
  if (target === "$terminal") {
    ...
    // outcome; vNext requires an explicit terminal status.
...
function convertWorkflowFile(
  ...
  for (const [definitionIndex, entry] of list.entries()) {
    ...
    // Hub/webhook-specific fields have no vNext workflow equivalent; the
    ...
    for (const [stageIndex, stageEntry] of rawStages.entries()) {
          ...
          if (!acceptedStatuses) {
            const chosen = resolveDecision(
              ...
              acceptedStatusesMalformed
                ? `evidence policy ${policyKey} acceptedStatuses is malformed (not an array) and was preserved in the report; confirm mapping the policy to vNext 'passed'`
                : `evidence policy ${policyKey} accepted statuses ${JSON.stringify(elide(accepted.map((candidate) => String(candidate)).join(","), 200))} cannot map losslessly; confirm mapping to vNext 'passed'`,
              ["passed"],
              decisions,
            );
      ...
      if (policyProducers.size > 0) {
        // vNext evidence producers must execute within the step assignment
      ...
      if (on) {
        ...
        for (const [rawOutcome, rawTransition] of Object.entries(on).sort(([left], [right]) => compareCodeUnits(left, right))) {
          ...
          if (seenOutcomes.has(outcome)) {
            // Two legacy outcome keys normalize to the same vNext outcome;
      ...
      if (typeof stage.maxTransitions === "number") {
        // Legacy per-stage transition budgets have no vNext equivalent;
    ...
    if (workflow.limits === undefined) {
      // vNext requires a global budget whenever any back-edge exists.
...
export interface VnextMigrationPlanResult {
  plan: JsonObject;
  resources: Map<string, JsonObject>;
  ambiguities: readonly VnextMigrationAmbiguity[];
  ...
  decisions: Readonly<Record<string, JsonValue>>;
  sources: readonly VnextLegacySource[];
}

export interface VnextMigrationOptions extends VnextConfigOptions {
  decisions?: Readonly<Record<string, JsonValue>>;
  projectId?: string;
  projectName?: string;
}
...
export function planVnextMigration(projectRoot: string, options: VnextMigrationOptions = {}): VnextMigrationPlanResult {
  ...
  const schemaIssues: VnextConfigIssue[] = [];
  const kindOf = (path: string): VnextResourceKind => path === ".kxm/project.yaml" ? "project"
  ...
  const semanticResources = new Map<string, { kind: VnextResourceKind; id?: string; value: JsonObject }>();
...
export function readVnextMigrationDecisions(path: string, schemasDir?: string): JsonObject {
  if (!existsSync(path)) {
    throw new VnextConfigError([migrateIssue("discovery", "decisions_missing", path, "migration decisions file does not exist")]);
  }
...
export interface VnextMigrationApplyResult {
...
export interface VnextMigrationApplyOptions extends VnextMigrationOptions {
  dryRun?: boolean;
...
export function applyVnextMigration(projectRoot: string, options: VnextMigrationApplyOptions = {}): VnextMigrationApplyResult {
  const root = migrationGitRoot(projectRoot);
  const storeOptions: VnextLocalBindingStoreOptions = {
  ...
  const execute = (lock: VnextLocalBindingLock | undefined): VnextMigrationApplyResult => {
    const receiptAbsolute = join(root, ...VNEXT_MIGRATION_RECEIPT_PATH.split("/"));
    ...
    const decisionsRecord = options.decisionsFile ? readVnextMigrationDecisions(options.decisionsFile, options.schemasDir) : undefined;
    ...
    if (Object.keys(decisions).length > 0) {
      ...
      if (unknown.length > 0) {
        throw new VnextConfigError(unknown.sort(compareCodeUnits).map((key) => migrateIssue("semantic", "decision_unknown", key, "decision key does not match any current ambiguity")));
      }
    ...
    // Refuse to overwrite any existing vNext state or legacy inputs.
    ...
    const parentIssues: VnextConfigIssue[] = [];
    ...
    try {
      ...
      const receipt: JsonObject = {
        ...
        decisionDigest: sha256Of(vnextCanonicalJson((decisionsRecord ?? { schema: "kxm.migration-decision.v1", projectId: first.plan.projectId, projectName: first.plan.projectName, sourceDigest: first.plan.sourceDigest, resolutions: decisions }) as JsonObject)),
...
export interface VnextMigrationVerifyResult {
  ok: boolean;
  projectRoot: string;
  receiptPath: string;
  configRevision?: string;
...
export function verifyVnextMigration(projectRoot: string, options: VnextConfigOptions = {}): VnextMigrationVerifyResult {
  const root = migrationGitRoot(projectRoot);
  const receiptAbsolute = join(root, ...VNEXT_MIGRATION_RECEIPT_PATH.split("/"));
  if (!existsSync(receiptAbsolute)) {
    return {
  ...
  try {
    ...
    if (!receipt) {
      if (legacyConfigFilesAt(root).length === 0) {
        ...
        // never consulted at load. Read it directly; readVnextMigrationReceipt
