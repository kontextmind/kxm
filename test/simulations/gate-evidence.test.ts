...
import { loadKxmProject, validateRunEvent, type JsonValue } from "../../plugins/kxm/src/project-config.ts";
import { kxmCanonicalJson } from "../../plugins/kxm/src/project-config.ts";
import { createKxmSimulatedProducer, driveKxmRun, pinKxmCompiledPlan, startKxmRun } from "../../plugins/kxm/src/engine.ts";
import {
...
import { gateDefinitionHash, gateRegistryHash } from "../../plugins/kxm/src/gate-hash.ts";
import {
...
function commandComplete(exitCode: number): KxmGateObservationInput {
...
function artifactsComplete(failedCount: number): KxmGateObservationInput {
  const now = new Date().toISOString();
  return {
    completeness: "complete",
    spawned: 0,
...
function noStartProof(stopCause: "none" | "cancel", errorClass: KxmGateObservationInput["errorClass"]): KxmGateObservationInput {
  const now = new Date().toISOString();
  return {
...
function incompleteProof(stopCause: KxmGateObservationInput["stopCause"]): KxmGateObservationInput {
  const now = new Date().toISOString();
  return {
...
function observationFromSql(row: Record<string, unknown>): KxmGateObservationRow {
  return {
    ...
    completeness: row.completeness as KxmGateObservationRow["completeness"],
    ...
    stopCause: row.stop_cause as KxmGateObservationRow["stopCause"],
    signalsAttempted: row.signals_attempted as KxmGateObservationRow["signalsAttempted"],
    errorClass: row.error_class as KxmGateObservationRow["errorClass"],
...
function rewriteObservation(path: string, mutate: (row: KxmGateObservationRow) => void): void {
  sql(path, (db) => {
...
async function assertReadsFail(context: KxmRuntimeContext, run: KxmRunRecord, code: string): Promise<void> {
  const matches = (error: unknown) => {
    assertCode(error, code);
    return true;
  };
  assert.throws(() => readKxmRunStatus(context, run), matches);
  assert.throws(() => cancelKxmRun(context, run.runId), matches);
  assert.throws(() => rebuildKxmRunProjection(context, run.runId), matches);
  await assert.rejects(
    () => driveKxmRun(context, run.runId, createKxmSimulatedProducer(async () => ({ outcome: "passed" }))),
    matches,
  );
}

function scanSecrets(events: readonly KxmRunEvent[]): string[] {
  const hits: string[] = [];
...
function appendRaw(context: KxmRuntimeContext, run: KxmRunRecord, eventType: string, payload: Record<string, unknown>): KxmRunEvent {
  const now = new Date().toISOString();
...
test("G2 envelope v2 pin and corrupt parse cases", () => {
  ...
  try {
    ...
    const context = openKxmRuntimeContext(root, { stateRoot, homeRuntimeId: HOME });
    try {
      const accepted = acceptKxmRun(context, bundle, { workflowId: "gate-command", prompt: "pin" });
      pinKxmCompiledPlan(context, bundle, accepted.run.runId);
      const run = context.eventStore.run(accepted.run.runId)!;
      const envelope = loadKxmRunPlanEnvelope(context.eventStore, run);
      ...
      const agent = acceptKxmRun(context, bundle, { workflowId: "agent-only", prompt: "nongate" });
      pinKxmCompiledPlan(context, bundle, agent.run.runId);
      const agentEnvelope = loadKxmRunPlanEnvelope(context.eventStore, context.eventStore.run(agent.run.runId)!);
      ...
      const corrupt = (mutate: (value: Record<string, unknown>) => void, pattern: RegExp) => {
        ...
        mutate(copy);
        assert.throws(() => parseKxmRunPlanEnvelope(JSON.stringify(copy), run, "sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"), pattern);
      };
      corrupt((value) => {
...
test("G3 legal command and artifact roundtrips through production builders", async () => {
  await withRun("gate-command", "pass", ({ context, run, envelope, bundle }) => {
    ...
    assert.equal(context.eventStore.runState(run.runId)!.state, kxmCanonicalJson(first.state as unknown as JsonValue));
    assert.equal(envelope.gates.registry?.hash, gateRegistryHash(bundle.gateRegistry!.value));
    assert.equal(
...
test("G3 reopen keeps run_state bytes and hashes", async () => {
  ...
  try {
    ...
    let bytes = "";
    try {
      const accepted = acceptKxmRun(context, bundle, { workflowId: "gate-command", prompt: "reopen" });
      runId = accepted.run.runId;
      pinKxmCompiledPlan(context, bundle, runId);
      startKxmRun(context, runId);
    ...
    try {
      ...
      assert.equal(readKxmRunStatus(reopened, run), "completed");
...
test("G4 row validation refuses extra keys, hashes, identity, and artifacts expect-fail", async () => {
  await withRun("gate-command", "row-invalid", ({ context, run, envelope }) => {
    ...
    expectInvalid(() => context.eventStore.insertGateAttempt({ ...attempt, extra: 1 } as KxmGateAttemptRow & { extra: number }));
    const missing = { ...attempt } as Record<string, unknown>;
    delete missing.contentHash;
    expectInvalid(() => context.eventStore.insertGateAttempt(missing as unknown as KxmGateAttemptRow));
    ...
    const observation: KxmGateObservationRow = {
    ...
    expectInvalid(() => context.eventStore.insertGateObservation({ ...hashed, stdout: "secret dump" } as KxmGateObservationRow & { stdout: string }));
  });
});
...
test("G6 on-disk corruption fails every production read", async () => {
  ...
  try {
    ...
    const context = openKxmRuntimeContext(root, { stateRoot, homeRuntimeId: HOME });
    try {
      const first = acceptKxmRun(context, bundle, { workflowId: "gate-command", prompt: "one" });
      pinKxmCompiledPlan(context, bundle, first.run.runId);
      startKxmRun(context, first.run.runId);
      const run1 = context.eventStore.run(first.run.runId)!;
      const envelope1 = loadKxmRunPlanEnvelope(context.eventStore, run1);
      ...
      const second = acceptKxmRun(context, bundle, { workflowId: "gate-command", prompt: "two" });
      pinKxmCompiledPlan(context, bundle, second.run.runId);
      startKxmRun(context, second.run.runId);
      const run2 = context.eventStore.run(second.run.runId)!;
      const envelope2 = loadKxmRunPlanEnvelope(context.eventStore, run2);
...
test("G7 stale attempt evidence cannot satisfy a retry", async () => {
  await withRun("gate-retry", "retry", async ({ context, run, envelope }) => {
    ...
    assert.equal(readKxmRunStatus(context, context.eventStore.run(run.runId)!), "running");
...
test("G7 copied attempt-1 evidence cannot satisfy attempt 2", async () => {
  await withRun("gate-retry", "copied-evidence", async ({ context, run, envelope }) => {
    ...
    const copiedObservation: KxmGateObservationRow = {
      ...sourceObservation,
...
test("G8 uncertainty freeze, no-start proof, and refusal after complete observation", async () => {
  await withRun("gate-command", "uncertain-intent", ({ context, run, envelope }) => {
    ...
    const cancelled = cancelKxmRun(context, run.runId);
    ...
    assert.throws(() => foldStoredKxmRun(context, context.eventStore.run(run.runId)!), /blocked_uncertain|run_events_illegal/);
  });
  ...
  await withRun("gate-command", "blocked-after-observed", ({ context, run, envelope }) => {
    ...
    foldStoredKxmRun(context, context.eventStore.run(run.runId)!);
    assert.throws(
  ...
  await withRun("gate-command", "fake-proof", ({ context, run, envelope }) => {
    ...
    assert.throws(() => readKxmRunStatus(context, context.eventStore.run(run.runId)!), /run_events_illegal|gate_observation_missing/);
  });
  ...
  await withRun("gate-command", "executing-unrecorded", ({ context, run, envelope }) => {
    ...
    assert.throws(() => readKxmRunStatus(context, context.eventStore.run(run.runId)!), /executing_unrecorded is illegal on a gate step/);
  });
});
...
test("G8 proof-only settlement cannot count as evaluated success", async () => {
  await withRun("gate-fail", "proof-not-expect-fail", ({ context, run, envelope }) => {
    ...
    assert.equal(context.eventStore.gateRowCounts(run.runId).evidence, 0);
    assert.equal(readKxmRunStatus(context, context.eventStore.run(run.runId)!), "failed");
...
test("G8 rehashed on-disk contradictions fail replay independently of insertion", async () => {
  const cases: Array<{ name: string; setup: "complete" | "no-start"; mutate: (row: KxmGateObservationRow) => void }> = [
...
test("G8 proof-only wrong hash, outcome-after-proof, and orphan evidence fail closed", async () => {
  ...
  await withRun("gate-command", "outcome-after-proof", ({ context, run, envelope }) => {
    ...
    assert.throws(() => readKxmRunStatus(context, context.eventStore.run(run.runId)!), /gate outcome requires evaluated effect.settled|run_events_illegal/);
  });
...
test("G6 observation owned by this run with a foreign attempt is visible", async () => {
  ...
  try {
    ...
    const context = openKxmRuntimeContext(root, { stateRoot, homeRuntimeId: HOME });
    try {
      const first = acceptKxmRun(context, bundle, { workflowId: "gate-command", prompt: "owner" });
      pinKxmCompiledPlan(context, bundle, first.run.runId);
      startKxmRun(context, first.run.runId);
      const run1 = context.eventStore.run(first.run.runId)!;
      const envelope1 = loadKxmRunPlanEnvelope(context.eventStore, run1);
      ...
      const second = acceptKxmRun(context, bundle, { workflowId: "gate-command", prompt: "other" });
      pinKxmCompiledPlan(context, bundle, second.run.runId);
      startKxmRun(context, second.run.runId);
      const run2 = context.eventStore.run(second.run.runId)!;
      const envelope2 = loadKxmRunPlanEnvelope(context.eventStore, run2);
...
test("G9 pin drift and coordinated rewrite trust boundary", async () => {
  await withRun("gate-command", "envelope-only", ({ context, run, envelope }) => {
    ...
    assert.throws(() => loadKxmRunPlanEnvelope(context.eventStore, context.eventStore.run(run.runId)!), /run_plan_corrupt/);
  });
  ...
  await withRun("gate-command", "pin-and-envelope", ({ context, run, envelope }) => {
    ...
    sql(context.eventStore.path, (db) => {
      ...
      const parsed = JSON.parse(row.envelope) as KxmRunPlanEnvelope;
      const mutated = {
        ...parsed,
        gates: { ...parsed.gates, controlRoot: { ...parsed.gates.controlRoot, projectKey: "drifted-key" } },
      };
      const hash = hashKxmRunPlanEnvelope(mutated);
      db.prepare("UPDATE run_plans SET envelope = ?, run_plan_hash = ? WHERE run_id = ?").run(kxmCanonicalJson(mutated as unknown as JsonValue), hash, run.runId);
    ...
    assert.throws(() => foldStoredKxmRun(context, context.eventStore.run(run.runId)!), /gate_attempt_pin_mismatch/);
  });
  ...
  await withRun("gate-command", "coordinated-rewrite", ({ context, run, envelope }) => {
    ...
    sql(context.eventStore.path, (db) => {
      ...
      const tsAttempt: KxmGateAttemptRow = {
        ...
        gateKind: attempt.gate_kind as KxmGateAttemptRow["gateKind"],
        expect: attempt.expect as KxmGateAttemptRow["expect"],
    ...
    assert.equal(readKxmRunStatus(context, context.eventStore.run(run.runId)!), "completed");
  });
  ...
  try {
    ...
    const context = openKxmRuntimeContext(root, { stateRoot, homeRuntimeId: HOME });
    try {
      const accepted = acceptKxmRun(context, bundle, { workflowId: "gate-command", prompt: "timeout-edit" });
      pinKxmCompiledPlan(context, bundle, accepted.run.runId);
      const before = loadKxmRunPlanEnvelope(context.eventStore, context.eventStore.run(accepted.run.runId)!);
...
`);
      ...
      const beforeRev = kxmPolicyRevisions(bundle);
      const afterRev = kxmPolicyRevisions(drifted);
      ...
      assert.throws(() => pinKxmCompiledPlan(context, drifted, accepted.run.runId), /run_revision_drift/);
    } finally {
      closeKxmRuntimeContext(context);
    }
  } finally {
...
test("G10 agent-only regressions still fold through the single production path", async () => {
  ...
  try {
    ...
    const context = openKxmRuntimeContext(root, { stateRoot, homeRuntimeId: HOME });
    try {
      const accepted = acceptKxmRun(context, bundle, { workflowId: "agent-only", prompt: "agent" });
      pinKxmCompiledPlan(context, bundle, accepted.run.runId);
      let index = 0;
      const outcomes = ["passed", "failed", "passed", "passed", "passed"];
      const driven = await driveKxmRun(context, accepted.run.runId, createKxmSimulatedProducer(async () => {
      ...
      assert.equal(kxmCanonicalJson(foldStoredKxmRun(context, context.eventStore.run(accepted.run.runId)!) as unknown as JsonValue), stored);
      const gated = acceptKxmRun(context, bundle, { workflowId: "unsupported-gate", prompt: "still-refused" });
      pinKxmCompiledPlan(context, bundle, gated.run.runId);
      startKxmRun(context, gated.run.runId);
      const blocked = await driveKxmRun(context, gated.run.runId, createKxmSimulatedProducer(async () => ({ outcome: "passed" })));
