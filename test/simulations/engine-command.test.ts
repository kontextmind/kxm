...
import { loadKxmProject, kxmCanonicalJson } from "../../plugins/kxm/src/project-config.ts";
import { recordGateIntent } from "../../plugins/kxm/src/engine-gate-records.ts";
import { loadKxmRunPlanEnvelope } from "../../plugins/kxm/src/engine-plan.ts";
import { computeKxmPermissionDiff } from "../../plugins/kxm/src/permission.ts";
import { kxmCommandGateSeams } from "../../plugins/kxm/src/engine-command.ts";
import {
  KxmRunScheduler,
...
function resetSeams(): void {
  kxmCommandGateSeams.timing = undefined;
  kxmCommandGateSeams.beforeSpawn = undefined;
  kxmCommandGateSeams.afterSpawned = undefined;
  kxmCommandGateSeams.afterStopRequested = undefined;
  kxmCommandGateSeams.afterChildClose = undefined;
  kxmGateDispatchSeams.afterEvaluate = undefined;
  kxmGateDispatchSeams.afterObservationWrite = undefined;
  kxmGateDispatchSeams.afterUncertaintyWrite = undefined;
  kxmGateDispatchSeams.afterSettlementCommit = undefined;
  kxmGateDispatchSeams.afterSpawnRecord = undefined;
  kxmGateDispatchSeams.beforeCommandSpawn = undefined;
  kxmGateDispatchSeams.beforeCommandSettle = undefined;
}
...
function observation(context: KxmRuntimeContext, runId: string) {
...
test("S4 admission hold is armed during spawn and released after evaluated settlement", async () => {
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    ...
    kxmCommandGateSeams.afterSpawned = () => {
      seen = kxmActiveScheduledRuns(context.eventStore.path);
      const hold = kxmGateHold(context.eventStore.path, run.runId);
      assert.equal(hold?.state, "active");
      assert.equal(hold?.attemptId, context.eventStore.gateAttemptsForRun(run.runId)[0]!.attemptId);
      busyCheck = assert.rejects(() => stepKxmRun(context, run.runId, producer()), /run_busy/);
    };
    const result = await stepKxmRun(context, run.runId, producer());
    assert.equal(result.state.status, "completed");
    await busyCheck;
    assert.equal(seen, 1);
    assert.equal(kxmActiveScheduledRuns(context.eventStore.path), 0);
    assert.equal(kxmGateHold(context.eventStore.path, run.runId), undefined);
  });
  ...
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    const scheduled = acceptKxmRun(context, bundle, { workflowId: "gate-command-ready", prompt: "hold-sched" });
...
test("S4 no-start covers missing executable, validation, and cancel after intent before spawn", async () => {
  ...
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    kxmCommandGateSeams.beforeSpawn = () => {
  ...
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    ...
    kxmGateDispatchSeams.beforeCommandSpawn = () => {
      cancelKxmRun(context, run.runId);
    };
    const result = await stepKxmRun(context, run.runId, producer());
...
test("S4 timeout, term-kill, and exit-0-during-grace stay uncertain", { timeout: 15_000 }, async () => {
  kxmCommandGateSeams.timing = { termGraceMs: 120, finalWaitMs: 120, lingerMs: 120 };
  ...
  await withS4({ argv: sleeper, timeoutMs: 200 }, async ({ context, bundle }) => {
    ...
    assert.equal(kxmGateHold(context.eventStore.path, run.runId)?.state, "unsettled");
  ...
  await withS4({ argv: ignoreTerm, timeoutMs: 200 }, async ({ context, bundle }) => {
    kxmCommandGateSeams.timing = { termGraceMs: 80, finalWaitMs: 120, lingerMs: 80 };
  ...
  await withS4({
    ...
    kxmCommandGateSeams.timing = { termGraceMs: 400, finalWaitMs: 120, lingerMs: 120 };
    ...
    const bundle2 = loadKxmProject(root);
    const run = acceptReady(context, bundle2, "gate-command-ready", "exit0-grace");
    const pending = stepKxmRun(context, run.runId, producer());
...
`],
    ...
    kxmCommandGateSeams.timing = { termGraceMs: 120, finalWaitMs: 200, lingerMs: 120 };
...
`, descendant, parent],
  ...
  await withS4({ argv: emptyScript(0), timeoutMs: 3_600_000 }, async ({ root, context, bundle }) => {
    kxmCommandGateSeams.timing = { termGraceMs: 80, finalWaitMs: 80, lingerMs: 80 };
...
test("S4 recording failures distinguish rollback from committed proof", async () => {
  await withS4({ argv: [NODE, "-e", "setInterval(() => {}, 1e9)"], timeoutMs: 200 }, async ({ context, bundle }) => {
    ...
    kxmGateDispatchSeams.afterSpawnRecord = () => {
      throw new Error("spawn record rolled back");
    };
    kxmCommandGateSeams.afterStopRequested = () => {
      holdAtStop = kxmGateHold(context.eventStore.path, run.runId)?.state;
    };
    const result = await stepKxmRun(context, run.runId, producer());
    assert.equal(result.handoff?.reason, "attempt_unsettled");
  ...
  await withS4({ argv: [NODE, "-e", "setInterval(() => {}, 1e9)"], timeoutMs: 200, maxConcurrentRuns: 2 }, async ({ context, bundle }) => {
    kxmCommandGateSeams.timing = { termGraceMs: 80, finalWaitMs: 80, lingerMs: 80 };
    kxmGateDispatchSeams.afterUncertaintyWrite = () => {
      throw new Error("uncertainty rolled back");
    };
    const run = acceptReady(context, bundle, "gate-command-ready", "uncertain-fail");
    await assert.rejects(() => stepKxmRun(context, run.runId, producer()), /uncertainty rolled back/);
    ...
    assert.equal(foldStoredKxmRun(context, context.eventStore.run(run.runId)!).currentStep?.effectState, "dispatched");
    assert.equal(context.eventStore.capabilityByAttempt(attempt.attemptId)?.state, "issued");
    assert.equal(kxmGateHold(context.eventStore.path, run.runId)?.state, "unsettled");
    await assert.rejects(() => stepKxmRun(context, run.runId, producer()), /run_busy/);
    const blocked = await stepKxmRun(
      context,
  ...
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    kxmGateDispatchSeams.beforeCommandSettle = () => {
  ...
  await withS4({ argv: emptyScript(0), maxConcurrentRuns: 1 }, async ({ context, bundle }) => {
    kxmGateDispatchSeams.afterSettlementCommit = () => {
      throw new Error("committed proof then failed");
    };
    const run = acceptReady(context, bundle, "gate-command-ready", "committed-proof");
    await assert.rejects(() => stepKxmRun(context, run.runId, producer()), /committed proof then failed/);
    const { row } = observation(context, run.runId);
    ...
    const admitted = await stepKxmRun(context, next.runId, producer());
...
test("S4 committed-proof active hold finishes for proof-only settlement and wakes the scheduler queue", async () => {
  await withS4({ argv: ["/no/such/kxm-s4-command"], maxConcurrentRuns: 1 }, async ({ context, bundle }) => {
    ...
    assert.equal(kxmGateHold(context.eventStore.path, next.runId), undefined);
    assert.equal(context.eventStore.gateRowCounts(run.runId).observations, 1);
  });
  ...
  await withS4({ argv: emptyScript(0), maxConcurrentRuns: 1 }, async ({ context, bundle }) => {
    let injected = 0;
    let queued: Promise<Awaited<ReturnType<KxmRunScheduler["enqueue"]>>> | undefined;
    const first = acceptKxmRun(context, bundle, { workflowId: "gate-command-ready", prompt: "queue-first" });
    const second = acceptKxmRun(context, bundle, { workflowId: "gate-command-ready", prompt: "queue-second" });
    ...
    assert.equal(kxmGateHold(context.eventStore.path, first.run.runId), undefined);
    assert.equal(kxmActiveScheduledRuns(context.eventStore.path), 0);
...
test("S4 post-commit hold stays when unsettled, mismatched, or unverifiable", async () => {
  await withS4({ argv: emptyScript(0), maxConcurrentRuns: 1 }, async ({ context, bundle }) => {
    ...
    kxmGateDispatchSeams.afterSettlementCommit = () => {
      const hold = kxmGateHold(context.eventStore.path, run.runId);
      const token = kxmAdmittedToken(context.eventStore.path, run.runId);
      assert.equal(hold?.state, "active");
      markKxmGateHoldUnsettled(context.eventStore.path, run.runId, token!, hold!.attemptId);
      throw new Error("committed proof then failed");
    };
    await assert.rejects(() => stepKxmRun(context, run.runId, producer()), /committed proof then failed/);
    const { row } = observation(context, run.runId);
    ...
    await assert.rejects(() => stepKxmRun(context, next.runId, producer()), /run_admission_exceeded/);
  });
  ...
  await withS4({ argv: emptyScript(0), maxConcurrentRuns: 1 }, async ({ context, bundle }) => {
    ...
    kxmCommandGateSeams.afterSpawned = () => {
      ...
      assert.equal(hold?.state, "active");
      assert.throws(
        () => finishKxmOwnedGate(context.eventStore.path, run.runId, "mismatched-token", hold!.attemptId),
        /owned gate finish requires the exact admitted token/,
      );
      assert.throws(
        () => finishKxmOwnedGate(context.eventStore.path, run.runId, hold!.token, "mismatched-attempt"),
        /owned gate finish does not match the armed attempt/,
      );
      assert.equal(kxmGateHold(context.eventStore.path, run.runId)?.state, "active");
    };
    const result = await stepKxmRun(context, run.runId, producer());
    assert.equal(result.state.status, "completed");
    assert.equal(kxmGateHold(context.eventStore.path, run.runId), undefined);
...
test("S4 context close during execution suppresses late writes and does not stop another context", { timeout: 15_000 }, async () => {
  ...
  try {
    ...
    try {
      ...
      const folded = foldStoredKxmRun(reopened, reopened.eventStore.run(run.runId)!);
      assert.ok(folded.currentStep?.effectState === "intent" || folded.currentStep?.effectState === "dispatched");
      assert.equal(kxmGateHold(reopened.eventStore.path, run.runId)?.state, "unsettled");
    } finally {
  ...
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    ...
    const result = await driveKxmRun(context, other.runId, producer());
    assert.equal(result.state.status, "completed");
  });
});
...
test("run-duration budget overrun during a command gate keeps budget_run_duration", { timeout: 15_000 }, async () => {
  await withS4({ argv: emptyScript(0) }, async ({ root, context }) => {
    ...
    const bundle = loadKxmProject(root);
    const accepted = acceptKxmRun(context, bundle, { workflowId: "gate-command-ready", prompt: "gate-budget" });
    pinKxmCompiledPlan(context, bundle, accepted.run.runId);
    kxmGateDispatchSeams.beforeCommandSettle = () => {
    ...
    const session = await KxmRunScheduler.for(context, bundle).openDriveSession(accepted.run.runId, {
      mode: "simulated",
...
test("S4 restart brake: in-process busy, durable recovery pending, controller without hold is not owned", async () => {
  ...
  await withS4({ argv: [NODE, "-e", "setInterval(() => {}, 1e9)"], timeoutMs: 200, maxConcurrentRuns: 2 }, async ({ context, bundle }) => {
    ...
    const blocked = await stepKxmRun(context, next.runId, producer());
    assert.equal(blocked.handoff?.reason, "gate_recovery_pending");
    assert.equal(kxmGateHold(context.eventStore.path, run.runId)?.state, "unsettled");
  });
  ...
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    ...
    const attempt = context.eventStore.gateAttemptsForRun(run.runId)[0]!;
    registerKxmAttemptController(context.eventStore.path, run.runId, {
    ...
    try {
      ...
      unregisterKxmAttemptController(context.eventStore.path, run.runId, attempt.attemptId);
    }
  });
});
...
test("S4 declarations refuse before intent; equal timeout proceeds; timeout-only still narrows permission", async () => {
  ...
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    ...
    const refused = await stepKxmRun(context, noRepos.runId, producer());
    ...
    const narrowResult = await stepKxmRun(context, narrow.runId, producer());
    ...
    const equalResult = await stepKxmRun(context, equal.runId, producer());
...
test("S4 complete rows carry no raw stdout/stderr text keys", async () => {
  await withS4({ argv: emptyScript(0) }, async ({ context, bundle }) => {
    ...
    await driveKxmRun(context, run.runId, producer());
    const { row } = observation(context, run.runId);
    assertNoRawText(row!);
    assert.equal(kxmCanonicalJson(row as never).includes("hello"), false);
  });
});
