...
import {
  computeKxmPermissionDiff,
  computeKxmResourcePermissionDiff,
  diffKxmProjectAgainstRevision,
  formatKxmPermissionDiff,
  loadKxmProjectAtRevision,
  kxmAuthorityEntries,
  kxmProseEntries,
} from "../../plugins/kxm/src/permission.ts";
import { KxmSchemaRegistry, loadKxmProject, type JsonObject } from "../../plugins/kxm/src/project-config.ts";
...
function committedProject(prefix: string): string {
  ...
  makeGitRoot(root);
  initializeKxmProject(root, { projectId: "prj_01JPERMISSIONTEST000000000", projectName: "Permission Test" });
  git(root, ["add", "-A"]);
...
test("secret grants key by ref so removing the first of two reports a single narrowing", () => {
  ...
  const removed = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, {
    ...base,
...
test("authority projection excludes prose and covers every authority-bearing field", () => {
  ...
  const entries = kxmAuthorityEntries({ kind: "agent", id: "coordinator", value: agent });
  ...
  const prose = kxmProseEntries({ kind: "agent", value: agent });
...
test("lattice orders: repository access, network, budgets, quorums, snapshots", () => {
  ...
  const accessUp = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, defaultRepositoryAccess: "write" });
  assert.equal(accessUp[0]!.direction, "expansion");
  assert.equal(accessUp[0]!.field, "repository-access");
  const accessDown = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, defaultRepositoryAccess: "none" });
  assert.equal(accessDown[0]!.direction, "narrowing");
  const netUp = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, network: "host" });
  assert.equal(netUp[0]!.direction, "expansion");
  const netDown = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, network: "none" });
  ...
  const budgetUp = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", budgetBase, { ...budgetBase, limits: { maxTransitions: 20, maxRunDurationMs: 1000 } });
  assert.equal(budgetUp[0]!.direction, "expansion");
  const budgetDown = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", budgetBase, { ...budgetBase, limits: { maxTransitions: 5, maxRunDurationMs: 1000 } });
  assert.equal(budgetDown[0]!.direction, "narrowing");
  const budgetMixed = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", budgetBase, { ...budgetBase, limits: { maxTransitions: 5, maxRunDurationMs: 2000 } });
  ...
  const weakDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", workflowWithPolicy, weakened);
  ...
  const degradedDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", workflowWithPolicy, degraded);
  ...
  const strongDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", workflowWithPolicy, strengthened);
  ...
  const snapshotUp = computeKxmResourcePermissionDiff("project", ".kxm/project.yaml", snapshotBase, {
...
test("secret grants, executors, models, transitions, and shapes classify conservatively", () => {
  ...
  const secretAdded = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, secrets: [...base.secrets as never[], { ref: "b", as: "B_TOKEN" }] });
  assert(secretAdded.some((change) => change.field === "secrets" && change.direction === "expansion"), "new secret grant expands");
  const secretRemoved = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, secrets: [] });
  assert(secretRemoved.some((change) => change.field === "secrets" && change.direction === "narrowing"), "removed grant narrows");
  const executorChanged = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, executor: "ssh" });
  assert.equal(executorChanged[0]!.direction, "expansion");
  const modelChanged = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, model: { provider: "xai", model: "grok-5" } });
  ...
  const bypassDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", transitionBase, bypassed);
...
test("prose-only changes are neutral; shape and transition changes are reviewed", () => {
  ...
  const prose = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, purpose: "new purpose" });
  assert.equal(prose.length, 1);
  assert.equal(prose[0]!.direction, "neutral");
  const unchanged = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, structuredClone(base));
...
test("trust diff against HEAD: clean tree is empty, expansions and narrowings classify", () => {
  ...
  try {
    const clean = diffKxmProjectAgainstRevision(root, "HEAD");
    assert.equal(clean.changes.length, 0);
    assert.equal(clean.requiresReview, false);
    assert.match(formatKxmPermissionDiff(clean), /no authority-bearing or prose changes/);

    editProject(root, (content) => content.replace("network: provider-only", "network: host"));
    const expanded = diffKxmProjectAgainstRevision(root, "HEAD");
    ...
    assert.match(formatKxmPermissionDiff(expanded), /EXPANSION.*network/);

    editProject(root, (content) => content.replace("network: host", "network: none"));
    const narrowed = diffKxmProjectAgainstRevision(root, "HEAD");
    ...
    const proseOnly = diffKxmProjectAgainstRevision(root, "HEAD");
...
test("trust diff requires a valid base revision and a loadable working tree", () => {
  ...
  try {
    assert.throws(() => diffKxmProjectAgainstRevision(root, "HEAD; rm -rf /"), /git_revision_invalid/);
    assert.throws(() => diffKxmProjectAgainstRevision(root, "nonexistent-ref"), /git_base_unavailable/);
    ...
    assert.throws(() => diffKxmProjectAgainstRevision(root, "HEAD"), (error: unknown) => {
...
test("removing an override with a broader fallback expands (Fable: fallback semantics)", () => {
  ...
  const removed = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", base, { ...base, repositories: {} });
  ...
  const narrow = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", baseNarrow, { ...baseNarrow, repositories: {} });
  ...
  const toolsRemoved = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", toolsBase, { schema: "kxm.agent.v1", purpose: "x" });
...
test("quorum gaps: join changes, oracle retargets, minimum lowering, reusable evidence all expand", () => {
  ...
  const joinDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", joinBase, joinWeakened);
  ...
  const oracleDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", oracleBase, retargeted);
  ...
  const minDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", reqBase, minLowered);
  ...
  const reuseDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", reqBase, reused);
...
test("trust base revision: HEAD~1 accepted, tree pinned, member worktree projects diff", () => {
  ...
  try {
    ...
    const diff = diffKxmProjectAgainstRevision(root, "HEAD~1");
  ...
  try {
    ...
    const diff = diffKxmProjectAgainstRevision(memberRoot, "HEAD");
    ...
    const expanded = diffKxmProjectAgainstRevision(memberRoot, "HEAD");
...
function cpFixture(root: string): void {
  cpSync("examples/project", root, { recursive: true });
...
test("malicious Git tree entries are rejected, never written outside the shadow", () => {
  ...
  try {
    ...
    assert.throws(
      () => loadKxmProjectAtRevision(root, outer.stdout.trim()),
...
test("environment value hashes are never emitted in the report", () => {
  ...
  const envChanged = computeKxmResourcePermissionDiff("environment", ".kxm/project/env.yaml", envBase, { ...envBase, values: { REGION: "eu-west", TIER: "prod" } });
...
test("quorum: reusable-evidence narrowing never masks a weakened producer policy", () => {
  ...
  const diff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", baseWorkflow, weakened);
  ...
  const narrowDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", baseWorkflow, narrowed);
...
test("scalar budgets: lowering attempts or timeouts narrows, raising expands", () => {
  ...
  const down = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", base, lowered);
  ...
  const up = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", base, raised);
...
test("removing a model resource expands (tag retargeting), removing an agent narrows", () => {
  ...
  try {
    ...
    rmSync(modelFile);
    const diff = diffKxmProjectAgainstRevision(root, "HEAD");
    ...
    rmSync(spareFile);
    const agentDiff = diffKxmProjectAgainstRevision(root, "HEAD");
...
test("host-local bound members materialize under the shadow and fail closed on missing revisions", () => {
  ...
  try {
    ...
    makeGitRoot(root);
    initializeKxmProject(root, { projectId: "prj_01JBOUNDTEST0000000000000", projectName: "Bound Test" });
    const projectFile = join(root, ".kxm", "project.yaml");
    ...
    const clean = diffKxmProjectAgainstRevision(root, "HEAD", { repositoryBindings: bindings });
    ...
    const expanded = diffKxmProjectAgainstRevision(root, "HEAD", { repositoryBindings: bindings });
...
test("candidate-added bound member reports as expansion, never a load failure", () => {
  ...
  try {
    ...
    const diff = diffKxmProjectAgainstRevision(root, "HEAD", { repositoryBindings: { api: member } });
...
test("gitignored nested member (pathHint without gitlink) fails with trust_scope_unsupported", () => {
  ...
  try {
    ...
    assert.throws(
      () => diffKxmProjectAgainstRevision(root, "HEAD"),
      (error: unknown) => {
        assert(error instanceof Error);
...
test("evidence minimum raise narrows; unknown residual policy fields expand", () => {
  ...
  const raiseDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", baseReq, raised);
  ...
  const residualDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", baseWf, futureField);
...
test("hostile base repository ids are rejected before any shadow write", () => {
  ...
  try {
    ...
    assert.throws(
      () => diffKxmProjectAgainstRevision(root, "HEAD~1"),
      (error: unknown) => {
        assert(error instanceof Error);
  ...
  try {
    ...
    assert.throws(
      () => diffKxmProjectAgainstRevision(reserved, "HEAD~1"),
      (error: unknown) => {
        assert(error instanceof Error);
...
test("narrowing early returns never bypass the residual expansion check", () => {
  ...
  const diff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", baseReq, raisedPlus);
  assert(diff.some((change) => change.field === "evidence-quorum" && change.direction === "expansion"),
  ...
  const policyDiff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", baseWf, policyRaisedPlus);
...
test("new eligible producer at the same quorum floor expands (more passing combinations)", () => {
  ...
  const diff = computeKxmResourcePermissionDiff("workflow", ".kxm/workflows/w.yaml", baseWf, widened);
  assert(diff.some((change) => change.field === "evidence-quorum" && change.direction === "expansion"),
...
test("secret grants: required normalization and environment name renames classify correctly", () => {
  ...
  const becameRequired = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", baseAgent, {
    ...baseAgent,
  ...
  const becameOptional = computeKxmResourcePermissionDiff("agent", ".kxm/agents/a.yaml", {
    ...baseAgent,
    secrets: [{ ref: "token", as: "TOKEN" }],
  }, baseAgent);
  ...
  const renamed = computeKxmResourcePermissionDiff("environment", ".kxm/project/env.yaml", baseEnv, {
    ...baseEnv,
