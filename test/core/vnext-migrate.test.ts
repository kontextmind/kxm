...
function allApprovals(plan: VnextMigrationPlanResult): Record<string, JsonValue> {
...
function decisionFile(plan: VnextMigrationPlanResult, resolutions: Record<string, JsonValue>): JsonObject {
  return {
...
test("migration plan converts roster and workflows with deterministic ambiguities", () => {
  ...
  try {
    const first = planVnextMigration(root);
    const second = planVnextMigration(root);
...
test("apply installs the validated target, writes a receipt, and verify passes", () => {
  ...
  try {
    ...
    assert.equal(planVnextInitialization(root).mode, "ready");

    const verify = verifyVnextMigration(root);
    assert.equal(verify.ok, true, JSON.stringify(verify.issues));

    // Re-apply is idempotent.
    const again = applyVnextMigration(root, { decisions });
...
test("apply through a decisions file binds project, sources, and allowed values", () => {
  ...
  try {
    ...
    const applied = applyVnextMigration(root, { decisionsFile: decisionsPath });
    assert.equal(applied.action, "applied");
    ...
    try {
      const plan2 = planVnextMigration(root2);
      ...
      assert.throws(
        () => applyVnextMigration(root2, { decisionsFile: badPath }),
...
test("verify detects source, target, and receipt tampering", () => {
  ...
  try {
    const plan = planVnextMigration(root);
    const applied = applyVnextMigration(root, { decisions: allApprovals(plan) });
    assert.equal(applied.action, "applied");
    assert.equal(verifyVnextMigration(root).ok, true);
    ...
    // reported by verify, while the bundle itself still loads (the vNext
    // target is authoritative after migration and may evolve through Git).
    assert.equal(loadVnextProject(root).configRevision !== undefined, true);
  } finally {
    cleanup(root);
  }
  ...
  try {
    const plan = planVnextMigration(root2);
    applyVnextMigration(root2, { decisions: allApprovals(plan) });
    // Legacy source edited after migration: the receipt pins it read-only.
    writeFileSync(join(root2, ".kxm", "config", "agents.json"), `${AGENTS}\n`, "utf8");
    const verify = verifyVnextMigration(root2);
    assert.equal(verify.ok, false);
    assert(verify.issues.some((issue) => issue.code === "migration_source_changed"), JSON.stringify(verify.issues));
    assert.throws(() => loadVnextProject(root2), (error) => issueCodes(error).includes("migration_source_changed"));
  } finally {
    cleanup(root2);
  }
  ...
  try {
    const plan = planVnextMigration(root3);
    applyVnextMigration(root3, { decisions: allApprovals(plan) });
    ...
    const verify = verifyVnextMigration(root3);
    assert.equal(verify.ok, false);
...
test("missing receipt keeps legacy/vNext coexistence fail-closed", () => {
  ...
  try {
    ...
    assert.throws(() => loadVnextProject(root), (error) => issueCodes(error).includes("legacy_vnext_conflict"));
    assert.equal(planVnextInitialization(root).mode, "migrate");
    // Re-apply refuses to overwrite the partial state.
    assert.throws(
      () => applyVnextMigration(root, { decisions: allApprovals(planVnextMigration(root)) }),
...
test("migration refuses case-fold identity collisions and symlinks", () => {
  ...
  try {
    const plan = planVnextMigration(colliding);
    ...
    const resolved = planVnextMigration(colliding, { decisions });
...
test("decisions resolve drops, coordinator choice, model mapping, and dry-run stays clean", () => {
  ...
  try {
    ...
    const resolved = planVnextMigration(root, { decisions });
    assert.equal(resolved.ambiguities.length, 0, JSON.stringify(resolved.ambiguities.map((ambiguity) => ambiguity.key)));
...
test("apply never writes through linked target parents and rolls back on failure", () => {
  ...
  try {
    ...
    const plan = planVnextMigration(root);
    assert.throws(
      () => applyVnextMigration(root, { decisions: allApprovals(plan) }),
...
test("decision-supplied identifier that is already taken stays an ambiguity, never a silent delete", () => {
  ...
  try {
    ...
    const resolved = planVnextMigration(root2, { decisions: decisions2 });
    ...
    const resolved2 = planVnextMigration(root2, { decisions: decisions2 });
...
test("apply ordering: receipt check precedes planning; dry run reports preflight failures", () => {
  ...
  try {
    ...
    const again = applyVnextMigration(root, {});
    assert.equal(again.action, "already-migrated");
    // Dry run on the migrated tree also reports already-migrated.
    const dryAgain = applyVnextMigration(root, { dryRun: true });
  ...
  try {
    ...
    const plan = planVnextMigration(blocked);
    assert.throws(
      () => applyVnextMigration(blocked, { decisions: allApprovals(plan), dryRun: true }),
      (error) => issueCodes(error).includes("migration_target_exists"),
    );
  } finally {
    cleanup(blocked);
...
test("receipt bound to a different project identity is rejected at load", () => {
  ...
  try {
    ...
    assert.throws(() => loadVnextProject(root), (error) => issueCodes(error).includes("migration_project_mismatch"));
  } finally {
    cleanup(root);
  }
});
...
test("agent permission changes are direction unknown; numeric budget decisions accept any in-range value", () => {
  ...
  try {
    ...
    try {
      ...
      assert.throws(
        () => applyVnextMigration(root2, { decisions: bad }),
        (error) => issueCodes(error).includes("decision_value_invalid"),
      );
    } finally {
      cleanup(root2);
...
test("migrate commands require an authoritative Git root", () => {
  ...
  try {
    ...
    assert.throws(() => planVnextMigration(root), (error) => issueCodes(error).includes("git_root_required"));
    assert.throws(() => applyVnextMigration(root, {}), (error) => issueCodes(error).includes("git_root_required"));
    assert.throws(() => verifyVnextMigration(root), (error) => issueCodes(error).includes("git_root_required"));
  } finally {
    cleanup(root);
  }
});
...
test("post-write failure rolls back installed files so retry can succeed", () => {
  ...
  try {
    ...
    assert.throws(
      () => applyVnextMigration(root, { decisions, repositoryBindings: { "not a repo id": "/tmp/nowhere" } }),
    ...
    const retry = applyVnextMigration(root, { decisions });
...
test("verify reports migration_source_missing when legacy sources are removed after migration", () => {
  ...
  try {
    ...
    // The migrated project itself still loads (legacy input is gone, vNext is authoritative).
    assert.equal(loadVnextProject(root).configRevision !== undefined, true);
  } finally {
    cleanup(root);
  }
...
test("approved drops remove duplicate workflows and stages without placeholder leakage", () => {
  ...
  try {
    const plan = planVnextMigration(stages);
    ...
    const decisions = allApprovals(plan);
    const applied = applyVnextMigration(stages, { decisions });
    assert.equal(applied.action, "applied", JSON.stringify(applied.plan?.ambiguities));
    const bundle = loadVnextProject(stages);
    const steps = bundle.workflows.get("fix")!.value.steps as JsonObject[];
...
test("out-of-range terminal-edge budgets are preserved and dropped; oversized names truncate safely", () => {
  ...
  try {
    const plan = planVnextMigration(huge);
    ...
    const applied = applyVnextMigration(huge, { decisions: allApprovals(plan) });
    assert.equal(applied.action, "applied");
  } finally {
    cleanup(huge);
  }
...
test("pending stage identity previews always satisfy the identifier grammar", () => {
  ...
  try {
    const plan = planVnextMigration(dup);
    assert(plan.ambiguities.some((ambiguity) => ambiguity.key.startsWith("identity-duplicate:step:")));
    const workflow = plan.resources.get(".kxm/workflows/fix.yaml") as JsonObject;
    for (const step of workflow.steps as JsonObject[]) {
...
test("leading-underscore legacy ids normalize cleanly and malformed accepted statuses require decisions", () => {
  ...
  try {
    const plan = planVnextMigration(malformed);
    assert(
...
test("preview ids with separators at the slice boundary stay grammar-valid", () => {
  ...
  try {
    const plan = planVnextMigration(longBoundary);
    const workflow = plan.resources.get(".kxm/workflows/fix.yaml") as JsonObject;
    for (const step of workflow.steps as JsonObject[]) {
...
test("oversized legacy source sets and BOM-prefixed JSON are handled deterministically", () => {
  ...
  try {
    ...
    assert.throws(() => planVnextMigration(many), (error) => issueCodes(error).includes("legacy_source_limit"));
  } finally {
    cleanup(many);
  }
  ...
  try {
    ...
    const plan = planVnextMigration(bom);
