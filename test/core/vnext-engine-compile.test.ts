...
import { parseRestrictedYaml, vnextCanonicalJson, type JsonObject, type JsonValue } from "../../plugins/kxm/src/vnext-config.ts";
import {
  VNEXT_COMPILED_WORKFLOW_SCHEMA,
  VNEXT_RESERVED_STEP_KINDS,
  VnextEngineCompileError,
  compileVnextWorkflow,
  type VnextCompiledPlan,
  type VnextCompiledStep,
  type VnextEngineCompileIssue,
} from "../../plugins/kxm/src/vnext-engine-compile.ts";

// Loader validation of these fixtures is covered in test/vnext-config.test.ts.
const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
const defaultPath = resolve(repoRoot, "examples/vnext/.kxm/workflows/default.yaml");
const fixPath = resolve(repoRoot, "examples/vnext/.kxm/workflows/fix.yaml");
const compileModulePath = resolve(repoRoot, "plugins/kxm/src/vnext-engine-compile.ts");

function loadWorkflow(file: string): JsonObject {
  return parseRestrictedYaml(readFileSync(file), file);
}

function compileFixture(id: "default" | "fix"): VnextCompiledPlan {
  const file = id === "default" ? defaultPath : fixPath;
  return compileVnextWorkflow({ id, value: loadWorkflow(file), logicalPath: file });
}
...
function compile(value: JsonObject, id = "case"): VnextCompiledPlan {
  return compileVnextWorkflow({ id, value });
}

function issuesOf(value: JsonObject, id = "case"): VnextEngineCompileIssue[] {
  try {
    compile(value, id);
  } catch (error) {
    assert(error instanceof VnextEngineCompileError, `expected VnextEngineCompileError, received ${String(error)}`);
    return [...error.issues];
  }
...
function canonical(value: unknown): string {
  return vnextCanonicalJson(asJson(value));
}
...
function stepOf(plan: VnextCompiledPlan, id: string): VnextCompiledStep {
...
test("default.yaml compiles to the slim four-step plan", () => {
  ...
  assert.equal(plan.schema, VNEXT_COMPILED_WORKFLOW_SCHEMA);
...
test("empty validated description, instructions, and logicalPath stay present; omitted fields stay absent", () => {
  ...
  const emptyPath = compileVnextWorkflow({
    id: "case",
    value: document([
      {
        id: "plan",
  ...
  const omittedPath = compileVnextWorkflow({
    id: "case",
    value: document([
      {
        id: "plan",
...
test("compile is deterministic and serializes by canonical JSON", () => {
  ...
  const first = compileVnextWorkflow({ id: "fix", value });
  const second = compileVnextWorkflow({ id: "fix", value });
...
test("compiled plans are frozen, null-prototype, and unaliased", () => {
  ...
  const plan = compileVnextWorkflow({ id: "default", value });
...
test("kind workflow is reserved and unknown kinds are rejected", () => {
  assert.deepEqual([...VNEXT_RESERVED_STEP_KINDS], ["workflow"]);
...
test("compile module has no I/O or runtime-store value imports", () => {
  ...
  try {
    ...
    assert.deepEqual(specifiers, [{ specifier: "./vnext-config.ts", typeOnly: true }]);
    const banned = ["node:fs", "node:child_process", "node:net", "./vnext-runtime-store.ts", "./vnext-runtime.ts"];
