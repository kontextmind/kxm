...
`,
    ...
    const context = openVnextRuntimeContext(env.root, { stateRoot: env.stateRoot, homeRuntimeId: HOME });
    try {
      const accepted = acceptVnextRun(context, bundle, { workflowId: "default", prompt: "default rework" });
      pinVnextCompiledPlan(context, bundle, accepted.run.runId);
...
test("Phase 3 Driver: default.yaml command gate uncertainty and recovery via retry to completion", async () => {
  ...
  try {
    ...
    const context = openVnextRuntimeContext(env.root, { stateRoot: env.stateRoot, homeRuntimeId: HOME });
    try {
      const accepted = acceptVnextRun(context, bundle, { workflowId: "default", prompt: "default uncertain" });
      pinVnextCompiledPlan(context, bundle, accepted.run.runId);

      const producer = createVnextSimulatedProducer(async () => ({ outcome: "passed" }));

      // Run until uncertain gate blocks
      const blocked = await driveVnextRun(context, accepted.run.runId, producer, { allowLimits: true });
      ...
      const recovered = recoverVnextRun(context, accepted.run.runId, { action: "retry", reason: "operator retry" });
      ...
      // Drive to completion
      const completed = await driveVnextRun(context, accepted.run.runId, producer, { allowLimits: true });
...
test("Phase 3 Driver: default.yaml command gate uncertainty and terminal recovery (fail and cancel)", async () => {
  ...
  try {
    ...
    try {
      ...
      const failRun = acceptVnextRun(context, bundle, { workflowId: "default", prompt: "fail run" });
      pinVnextCompiledPlan(context, bundle, failRun.run.runId);
      const blockedFail = await driveVnextRun(context, failRun.run.runId, producer, { allowLimits: true });
      ...
      const failedRecovery = recoverVnextRun(context, failRun.run.runId, { action: "fail", reason: "operator aborted" });
      ...
      const cancelRun = acceptVnextRun(context, bundle, { workflowId: "default", prompt: "cancel run" });
      pinVnextCompiledPlan(context, bundle, cancelRun.run.runId);
      const blockedCancel = await driveVnextRun(context, cancelRun.run.runId, producer, { allowLimits: true });
      ...
      const cancelRecovery = recoverVnextRun(context, cancelRun.run.runId, { action: "cancel", reason: "operator cancelled" });
...
test("Phase 3 Driver: fix.yaml full drive (approval, two-producer join, rework loops on critics and approval)", async () => {
  ...
  try {
    ...
    const context = openVnextRuntimeContext(env.root, { stateRoot: env.stateRoot, homeRuntimeId: HOME });
    try {
      const accepted = acceptVnextRun(context, bundle, { workflowId: "fix", prompt: "fix run" });
      pinVnextCompiledPlan(context, bundle, accepted.run.runId);
...
test("Phase 3 Driver: Caller-authored and untrusted producers are rejected", async () => {
  ...
  try {
    ...
    const context = openVnextRuntimeContext(env.root, { stateRoot: env.stateRoot, homeRuntimeId: HOME });
    try {
      const accepted = acceptVnextRun(context, bundle, { workflowId: "default", prompt: "untrusted" });
      pinVnextCompiledPlan(context, bundle, accepted.run.runId);
      ...
      await assert.rejects(
        () => driveVnextRun(context, accepted.run.runId, fakeProducer, { allowLimits: true }),
