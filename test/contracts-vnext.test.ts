...
const schemaDir = resolve(root, "schemas/vnext");
const exampleDir = resolve(root, "examples/vnext");
...
function validateResource(
  ...
  const validate = ajv.getSchema(`https://schemas.kxm.dev/vnext/${name}.schema.json`);
...
test("vNext JSON Schemas compile with unique identities", () => {
...
test("vNext YAML project fixture validates and resolves semantically", () => {
...
test("vNext representative durable records validate", () => {
  ...
  for (const file of recordFiles) {
    ...
    const validate = ajv.getSchema(`https://schemas.kxm.dev/vnext/${schemaName}.schema.json`);
    assert(validate);
...
test("vNext records preserve source-event identity and complete repository pins", () => {
...
test("schemas reject unsafe effects, inconsistent delivery, unbounded snapshots, and ambiguous terminals", () => {
  const { ajv } = createValidator();
  const validateEffect = ajv.compile({ $ref: "https://schemas.kxm.dev/vnext/common.schema.json#/$defs/effectPolicy" });
  ...
  const validateDelivery = ajv.getSchema("https://schemas.kxm.dev/vnext/delivery-manifest.schema.json");
  ...
  const validateProject = ajv.getSchema("https://schemas.kxm.dev/vnext/project.schema.json");
  ...
  const validateWorkflow = ajv.getSchema("https://schemas.kxm.dev/vnext/workflow.schema.json");
  ...
  const validateEnvironment = ajv.getSchema("https://schemas.kxm.dev/vnext/environment.schema.json");
  ...
  const validateRunEvent = ajv.getSchema("https://schemas.kxm.dev/vnext/run-event.schema.json");
...
test("portable path schema rejects traversal and destination-incompatible names", () => {
  const { ajv } = createValidator();
  const validate = ajv.compile({ $ref: "https://schemas.kxm.dev/vnext/common.schema.json#/$defs/relativePath" });
...
test("sync-safe schema rejects raw or host-sensitive escape fields", () => {
  ...
  const validate = ajv.getSchema("https://schemas.kxm.dev/vnext/sync-event.schema.json");
  assert(validate);
...
test("migration plan, decision, and receipt contracts validate and fail closed", () => {
  ...
  const validatePlan = ajv.getSchema("https://schemas.kxm.dev/vnext/migration-plan.schema.json");
  assert(validatePlan);
  ...
  const validateDecision = ajv.getSchema("https://schemas.kxm.dev/vnext/migration-decision.schema.json");
  ...
  const validateReceipt = ajv.getSchema("https://schemas.kxm.dev/vnext/migration-receipt.schema.json");
