...
| Production vNext restricted loader, deterministic bundle hashing, Git discovery, fail-closed semantics, init classification (including empty/runtime-only `.kxm` create merge), provenance-tracked atomic creation, exact three-way repair, authority-change blocking, pinned crash resumption, shadow validation, explicit join, Runtime-local bindings, CLI isolation, and idempotence | `test/core/vnext-config.test.ts`, `test/core/cli.test.ts`, `test/core/package-install.test.ts` |
