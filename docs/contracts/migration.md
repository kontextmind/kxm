...
| SQLite store stamped with an older `user_version` | Refused with `runtime_schema_outdated`; `user_version` is left untouched, so the failure stays loud. Delete the state file (or re-run `kxm init`). Forward-only stamping is prohibited — it turns a clean failure into a later query against a column that does not exist. |
