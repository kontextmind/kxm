...
> [!IMPORTANT]
> The files are the authority, not this page: `.kxm/routes.yaml`, `.kxm/roles/`, `the role and model files` and `.kxm/prices.yaml`. Product routing decisions are recorded under Tracking → Decided in `plans/implementation-plan.md`.
...
## The developer roster (`the role and model files`)

The issue-127 runner (`just assign`, see the [assignment runner](assignment-runner.md)) uses its own policy file, [`the assembled developer policy`](../reference/config-reference.md#kxmrosteryaml-kxmdeveloper policyv1). Routes there name the harness, the model and the vendor explicitly:
...
| Symptom | Cause | Fix |
|---|---|---|
| `pi brake: xai has a native harness; refusing Pi impersonation` | A dev-helper request routed a native vendor through Pi. | Use the native harness recipe instead: `kxm lane run`, `just plan`, `kxm lane run` or `kxm lane run`. |
| `Roster policy refused: native vendor cannot use Pi` | A `the role and model files` Pi route names a native vendor, as in `openrouter/x-ai/…` or `nous-portal/anthropic/…`. | Remove the route. Only a native harness route is valid for that vendor. |
...
- [Configuration file reference](../reference/config-reference.md#kxmrosteryaml-kxmdeveloper policyv1): the `the assembled developer policy` fields
