...
| `kxm tenant status` | One composed read for machine clients (the portal backend): hub metadata (agent roster, message queue, hub's run **projection**) and the Runtime's **authoritative** run state on this box, each value labelled with its source; an unreadable upstream is reported `unavailable` with a stable reason (`hub_unreachable`, `hub_unauthorized`, `runtime_supervisor_not_running`) instead of being filled from the other source, disagreements are listed as `discrepancies`, and `degraded` flags a partial read. Attaches to a live supervisor and never starts one. Requires a KXM project; hub read uses the same admin-token precedence as `kxm hub bind` |
